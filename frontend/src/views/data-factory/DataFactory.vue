...
const getToolDisplayName = (toolName) => {
  return (
    t(`dataFactory.tools.${toolName}`) ||
    getToolDisplayNameOld(toolName) ||
    toolName
  );
};
...
const getToolDescription = (toolName) => {
  return (
    t(`dataFactory.toolDescs.${toolName}`) ||
    getToolDescriptionOld(toolName) ||
    ""
  );
};
...
const buildInputData = () => {
  ...
  if (category === "test_data") {
    ...
    if (toolName === "generate_chinese_address")
      data.full_address = form.full_address;
    if (toolName === "generate_company_name")
  ...
  if (category === "string") {
    ...
    if (toolName === "replace_string")
      return {
        text: form.text,
        old_str: form.old_str,
        new_str: form.new_str,
        is_regex: form.is_regex,
      };
    if (toolName === "escape_string")
      return { text: form.text, escape_type: form.escape_type };
    if (toolName === "unescape_string")
      return { text: form.text, unescape_type: form.unescape_type };
    if (toolName === "word_count") return { text: form.text };
    if (toolName === "text_diff")
      return { text1: form.text1, text2: form.text2 };
    ...
    if (toolName === "case_convert")
      return { text: form.text, convert_type: form.convert_type };
    if (toolName === "string_format")
      return { text: form.text, format_type: form.format_type };
  }

  if (category === "random") {
    if (toolName === "random_int")
      return {
        min_val: form.min_val,
        max_val: form.max_val,
        count: form.count,
      };
    if (toolName === "random_float")
      return {
        min_val: form.min_val,
        max_val: form.max_val,
    ...
    if (toolName === "random_string")
      return {
        length: form.length,
        char_type: form.char_type,
        count: form.count,
      };
    if (toolName === "random_uuid")
      return { version: form.version, count: form.count };
    if (toolName === "random_mac_address")
      return { separator: form.separator, count: form.count };
    if (toolName === "random_ip_address")
      return { ip_version: form.ip_version, count: form.count };
    ...
    if (toolName === "random_color")
      return { format: form.format, count: form.count };
    ...
    if (toolName === "random_sequence") {
      const sequence = form.sequence
        ? form.sequence.split(",").map((s) => s.trim())
        : [];
  ...
  if (category === "encoding") {
    if (toolName === "generate_barcode")
      return { data: form.data, barcode_type: form.barcode_type };
    if (toolName === "generate_qrcode")
      return { data: form.data, image_size: form.image_size, border: 4 };
    if (toolName === "decode_qrcode")
      return { image_data: form.image_data || "", image_format: "png" };
    if (toolName === "timestamp_convert")
      return {
        timestamp: form.timestamp,
        convert_type: form.timestamp_convert_type,
        timestamp_unit: form.timestamp_unit,
      };
    if (toolName === "base_convert")
      return {
        number: form.number,
        from_base: form.from_base,
        to_base: form.to_base,
      };
    if (toolName === "unicode_convert")
      return { text: form.text, convert_type: form.unicode_convert_type };
    if (toolName === "ascii_convert")
      return { text: form.text, convert_type: form.convert_type };
    if (toolName === "color_convert")
      return {
        color: form.color,
        from_type: form.from_type,
        to_type: form.to_type,
      };
    if (toolName === "url_encode")
      return { data: form.data, encoding: form.encoding, plus: form.plus };
    if (toolName === "url_decode")
      return { data: form.data, encoding: form.encoding, plus: form.plus };
    if (toolName === "jwt_decode")
      return { token: form.token, verify: form.verify, secret: form.secret };
    if (toolName === "image_to_base64")
      return {
        image_data: form.image_data || "",
        image_format: form.image_format || "png",
        include_prefix: form.include_prefix !== false,
      };
    if (toolName === "base64_to_image")
      return { base64_str: form.base64_str || "" };
    if (toolName === "base64_encode")
      return { text: form.text, encoding: form.encoding };
  ...
  if (category === "encryption") {
    ...
    if (toolName === "hash_comparison")
      return {
        text: form.text,
        hash_value: form.hash_value,
        algorithm: form.algorithm,
      };
    if (toolName === "aes_encrypt")
      return { text: form.text, password: form.password, mode: form.mode };
    if (toolName === "aes_decrypt")
      return {
        encrypted_text: form.text,
  ...
  if (category === "json") {
    if (toolName === "format_json")
      return {
        json_str: form.json_str,
        indent: form.indent,
        sort_keys: form.sort_keys,
        compress: form.compress,
      };
    if (toolName === "validate_json") return { json_str: form.json_str };
    if (toolName === "json_to_xml")
      return { json_str: form.json_str, root_tag: form.root_tag || "root" };
    ...
    if (toolName === "json_diff_enhanced")
      return {
        json_str1: form.json_str1,
        json_str2: form.json_str2,
        ignore_whitespace: form.ignore_whitespace,
        show_only_diff: form.show_only_diff,
      };
    if (toolName === "jsonpath_query")
      return { json_str: form.json_str, jsonpath_expr: form.jsonpath_expr };
    if (toolName === "json_path_list") return { json_str: form.json_str };
    if (toolName === "json_flatten")
      return { json_str: form.json_str, separator: form.separator || "." };
  }

  if (category === "crontab") {
    if (toolName === "generate_expression")
      return {
        minute: form.minute || "*",
        hour: form.hour || "*",
        day: form.day || "*",
    ...
    if (toolName === "parse_expression")
      return { expression: form.expression || "" };
    if (toolName === "get_next_runs")
      return { expression: form.expression || "", count: form.count || 10 };
    if (toolName === "validate_expression")
      return { expression: form.expression || "" };
  }
