  ...
  <div class="page-container">
    ...
    <div class="filter-bar">
      <el-form :inline="true" :model="filters" class="filter-form">
        ...
        <el-form-item :label="$t('reviewList.status')">
          ...
          >
            <el-option
              :label="$t('reviewList.statusPending')"
              value="pending"
            />
            <el-option
              :label="$t('reviewList.statusInProgress')"
              value="in_progress"
            />
            <el-option
              :label="$t('reviewList.statusApproved')"
              value="approved"
            />
            <el-option
              :label="$t('reviewList.statusRejected')"
              value="rejected"
            />
            <el-option
              :label="$t('reviewList.statusCancelled')"
              value="cancelled"
            />
          </el-select>
        </el-form-item>
    ...
    <div class="table-container">
      <el-table :data="reviews" v-loading="loading" stripe>
        <el-table-column
          prop="title"
          :label="$t('reviewList.reviewTitle')"
          min-width="200"
        ...
        <el-table-column
          :label="$t('reviewList.actions')"
          width="200"
          fixed="right"
        >
          <template #default="{ row }">
            <el-button link type="primary" @click="viewReview(row.id)">{{
              $t("reviewList.detail")
            }}</el-button>
            <el-button
              v-if="canReview(row)"
    ...
    <el-dialog
      ...
      :title="$t('reviewList.submitReview')"
      width="600px"
    >
        ...
        <el-form-item :label="$t('reviewList.reviewResult')" required>
          <el-radio-group v-model="reviewForm.status">
            <el-radio-button label="approved">{{
              $t("reviewList.approved")
            }}</el-radio-button>
            <el-radio-button label="rejected">{{
              $t("reviewList.rejected")
            }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
...
const confirmSubmitReview = async () => {
  try {
    await api.post(
      `/reviews/reviews/${currentReview.value.id}/submit_review/`,
      reviewForm,
    );
...
const canReview = (review) => {
  return review.assignments?.some(
    (a) => a.reviewer.id === userStore.user?.id && a.status === "pending",
  );
};

const canEdit = (review) => {
  return (
    review.creator.id === userStore.user?.id &&
...
const canDelete = (review) => {
  return (
    review.creator.id === userStore.user?.id && review.status === "pending"
  );
};
