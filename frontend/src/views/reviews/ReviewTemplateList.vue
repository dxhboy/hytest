<template>
  <div class="page-container">
      ...
      <el-form :inline="true" :model="filters" class="filter-form">
        <el-form-item :label="$t('reviewTemplate.project')">
          <el-select
            ...
            :placeholder="$t('reviewTemplate.selectProject')"
            clearable
        ...
        <template #header>
          <div class="card-header">
            ...
            <div class="card-actions">
              <el-button link type="primary" @click="useTemplate(template)">{{
                $t("reviewTemplate.useTemplate")
              }}</el-button>
              <el-button link type="warning" @click="editTemplate(template)">{{
                $t("reviewTemplate.edit")
              }}</el-button>
              <el-popconfirm
        ...
        <div class="template-content">
          ...
          <div class="template-checklist">
            <div class="checklist-title">
              {{ $t("reviewTemplate.checklistTitle") }}
            </div>
            <ul v-if="template.checklist?.length">
              <li
                v-for="(item, index) in template.checklist.slice(0, 3)"
                :key="index"
              >
                {{ item }}
              </li>
              <li v-if="template.checklist.length > 3" class="more-items">
                {{
                  $t("reviewTemplate.moreItems", {
                    count: template.checklist.length - 3,
                  })
                }}
              </li>
            </ul>
            <div v-else class="empty-checklist">
              {{ $t("reviewTemplate.noChecklist") }}
            </div>
          </div>

          <div class="template-reviewers">
            <div class="reviewers-title">
              {{ $t("reviewTemplate.reviewersTitle") }}
            </div>
            <div class="reviewers-list">
              ...
              <span
                v-if="!template.default_reviewers?.length"
                class="empty-reviewers"
              >
    ...
    <el-dialog
      ...
      :title="
        isEdit
          ? $t('reviewTemplate.editTitle')
          : $t('reviewTemplate.createTitle')
      "
      width="800px"
    ...
    >
      ...
      >
        <el-row :gutter="24">
          <el-col :span="12">
            <el-form-item
              :label="$t('reviewTemplate.templateName')"
              prop="name"
            >
              <el-input
                v-model="templateForm.name"
                :placeholder="$t('reviewTemplate.templateNamePlaceholder')"
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item
              :label="$t('reviewTemplate.associatedProject')"
              prop="project"
            >
              <el-select
      ...
      <template #footer>
        <el-button @click="templateDialogVisible = false">{{
          $t("reviewTemplate.cancel")
        }}</el-button>
        <el-button type="primary" @click="saveTemplate" :loading="saving">{{
          $t("reviewTemplate.save")
        }}</el-button>
      </template>
    </el-dialog>
  </div>
...
const editTemplate = (template) => {
  ...
  templateForm.checklist = template.checklist.length
    ? [...template.checklist]
    : [""];
...
const saveTemplate = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await api.put(
        `/reviews/review-templates/${editingTemplateId.value}/`,
        data,
      );
    ...
    ElMessage.error(
      isEdit.value
        ? t("reviewTemplate.updateFailed")
        : t("reviewTemplate.createFailed"),
    );
  } finally {
    saving.value = false;
  }
