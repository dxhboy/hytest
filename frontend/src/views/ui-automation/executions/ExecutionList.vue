  ...
  <div class="page-container">
    ...
    <div class="card-container">
      <div class="filter-bar">
        <el-form :inline="true" :model="queryParams" class="demo-form-inline">
          ...
          <el-form-item :label="$t('uiAutomation.common.status')">
            ...
            >
              <el-option
                :label="$t('uiAutomation.status.pending')"
                value="pending"
              />
              <el-option
                :label="$t('uiAutomation.status.running')"
                value="running"
              />
              <el-option
                :label="$t('uiAutomation.status.passed')"
                value="passed"
              />
              <el-option
                :label="$t('uiAutomation.status.failed')"
                value="failed"
              />
              <el-option
                :label="$t('uiAutomation.status.error')"
                value="error"
              />
            </el-select>
          </el-form-item>
          <el-form-item :label="$t('uiAutomation.execution.browserFilter')">
            <el-select
              ...
              :placeholder="$t('uiAutomation.execution.browserFilter')"
              clearable
            >
      ...
      >
        ...
        <el-table-column
          prop="test_case_name"
          :label="$t('uiAutomation.execution.caseName')"
          min-width="200"
        >
        ...
        <el-table-column
          :label="$t('uiAutomation.execution.relatedObject')"
          width="100"
          align="center"
        >
          <template #default="{ row }">
            <el-tag v-if="!row.test_suite" type="info" size="small">{{
        ...
        <el-table-column
          prop="status"
          :label="$t('uiAutomation.execution.statusFilter')"
          width="100"
          align="center"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          prop="engine"
          :label="$t('uiAutomation.execution.testEngine')"
          width="120"
          align="center"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          prop="headless"
          :label="$t('uiAutomation.execution.executionMode')"
          width="100"
          align="center"
        >
          <template #default="{ row }">
            {{
              row.headless
                ? $t("uiAutomation.execution.headlessMode")
                : $t("uiAutomation.execution.headedMode")
            }}
          </template>
        </el-table-column>
        <el-table-column
          prop="browser"
          :label="$t('uiAutomation.execution.browserFilter')"
          width="100"
          align="center"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          prop="started_at"
          :label="$t('uiAutomation.execution.startTime')"
          width="180"
          align="center"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          prop="finished_at"
          :label="$t('uiAutomation.execution.endTime')"
          width="180"
          align="center"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          :label="$t('uiAutomation.execution.duration')"
          width="120"
          align="center"
        >
          <template #default="{ row }">
      ...
      <div v-if="currentExecution" class="execution-detail">
        ...
        <el-descriptions :column="2" border>
          <el-descriptions-item
            :label="$t('uiAutomation.execution.caseName')"
            >{{ currentExecution.test_case_name }}</el-descriptions-item
          >
          <el-descriptions-item
            :label="$t('uiAutomation.execution.statusFilter')"
          >
          ...
          <el-descriptions-item
            :label="$t('uiAutomation.execution.browserFilter')"
            >{{
              getBrowserText(currentExecution.browser)
            }}</el-descriptions-item
          >
          <el-descriptions-item
            :label="$t('uiAutomation.execution.executor')"
            >{{ currentExecution.created_by_name }}</el-descriptions-item
          >
          <el-descriptions-item
            :label="$t('uiAutomation.execution.startTime')"
            >{{
          ...
          >
          <el-descriptions-item :label="$t('uiAutomation.execution.endTime')">{{
          ...
          <el-descriptions-item
            ...
            :span="2"
            >{{
              formatDuration(currentExecution.execution_time)
            }}</el-descriptions-item
          >
        </el-descriptions>
        ...
        >
          <!-- 执行日志 - 所有状态都显示 -->
          <el-tab-pane
            :label="$t('uiAutomation.execution.executionLogs')"
            name="logs"
          >
            <div class="logs-container">
              <div v-if="currentExecution.execution_logs">
                <div
                  v-for="(step, index) in parseExecutionLogs(
                    currentExecution.execution_logs,
                  )"
                  :key="index"
                  class="log-item"
                >
              ...
              <el-empty
                v-else
                :description="$t('uiAutomation.execution.noLogs')"
              />
            </div>
          </el-tab-pane>
          ...
          <!-- 失败截图 - 仅失败或错误状态显示 -->
          <el-tab-pane
            :label="$t('uiAutomation.execution.failedScreenshots')"
            name="screenshots"
            v-if="
          ...
          >
            <div class="screenshots-container">
                ...
                >
                  <h5>
                    {{
                      ...
                      `${$t("uiAutomation.execution.screenshot")} ${index + 1}`
                    }}
                  </h5>
                  <!-- 检查截图URL是否有效 -->
                  ...
                  <div v-else class="screenshot-error">
                    <el-icon><WarningFilled /></el-icon>
                    <span
                      >{{ $t("uiAutomation.execution.screenshotFailed")
                      }}{{
                        screenshot.error ||
                        $t("uiAutomation.execution.unknownReason")
                      }}</span
                    >
                  </div>
              ...
              <el-empty
                v-else
                :description="$t('uiAutomation.execution.noScreenshots')"
              />
            </div>
          </el-tab-pane>
          ...
          <!-- 错误信息 - 仅失败或错误状态显示 -->
          <el-tab-pane
            :label="$t('uiAutomation.execution.errorInfo')"
            name="error"
            v-if="
              currentExecution.status === 'failed' ||
              currentExecution.status === 'error'
          ...
          >
            <div class="errors-container">
              ...
              <el-empty
                v-else
                :description="$t('uiAutomation.execution.noError')"
              />
            </div>
          </el-tab-pane>
        </el-tabs>
    ...
    <!-- 重跑测试用例对话框 -->
    <el-dialog
      v-model="showRerunDialogVisible"
      :title="$t('uiAutomation.execution.rerunTitle')"
      width="500px"
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleRerun" :loading="rerunning">{{
          $t("uiAutomation.execution.confirmRerun")
        }}</el-button>
      </template>
    </el-dialog>
  </div>
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    t("uiAutomation.execution.messages.deleteConfirm"),
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(
    t("uiAutomation.execution.messages.batchDeleteConfirm", {
    ...
    try {
      ...
      ElMessage.success(
        t("uiAutomation.execution.messages.batchDeleteSuccess"),
      );
      selectedIds.value = [];
      loadExecutions();
    } catch (error) {
...
const handleRerun = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      ElMessage.warning(
        t("uiAutomation.execution.messages.rerunCompleteWithFailure") +
          ": " +
          (response.data.errors?.[0]?.message ||
            t("uiAutomation.execution.messages.viewDetails")),
      );
    }
  } catch (error) {
    showRerunDialogVisible.value = false;
    ElMessage.error(
      t("uiAutomation.execution.messages.rerunFailed") +
        ": " +
