<template>
  <div class="page-container">
    <div class="page-header">
      <h1 class="page-title">
        {{ $t("uiAutomation.ai.executionRecords.title") }}
      </h1>
    ...
    <div class="card-container">
      ...
      >
        ...
        <el-table-column
          :label="$t('uiAutomation.ai.executionRecords.serialNumber')"
          width="80"
        >
        ...
        <el-table-column
          prop="case_name"
          :label="$t('uiAutomation.ai.executionRecords.caseName')"
          min-width="200"
        ...
        <el-table-column
          prop="status"
          :label="$t('uiAutomation.ai.executionRecords.status')"
          width="120"
        >
        ...
        <el-table-column
          prop="duration"
          :label="$t('uiAutomation.ai.executionRecords.durationSeconds')"
          width="120"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          ...
          :label="$t('uiAutomation.ai.executionRecords.executor')"
          width="120"
        />
        <el-table-column
    ...
    >
      <div v-if="currentRecord" class="record-detail">
        <div class="detail-item">
          <span class="label"
            >{{ $t("uiAutomation.ai.executionRecords.caseName") }}:</span
          >
        ...
        <div class="detail-item">
          <span class="label"
            >{{ $t("uiAutomation.ai.executionRecords.status") }}:</span
          >
        ...
        <div class="detail-item">
          <span class="label"
            >{{ $t("uiAutomation.ai.executionRecords.duration") }}:</span
          >
          <span>{{
            currentRecord.duration
              ? currentRecord.duration.toFixed(2) +
                " " +
                $t("uiAutomation.ai.executionRecords.seconds")
              : $t("uiAutomation.ai.executionRecords.unknown")
          }}</span>
        </div>
        ...
        <div v-if="currentRecord.task_description" class="detail-item mt-15">
          <span class="label"
            >{{ $t("uiAutomation.ai.executionRecords.taskDescription") }}:</span
          >
        </div>
        <div
          v-if="currentRecord.task_description"
        ...
        <div class="detail-item mt-15">
          <span class="label"
            >{{ $t("uiAutomation.ai.executionRecords.executionLogs") }}:</span
          >
        </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="success" @click="openReportFromDetail">{{
            $t("uiAutomation.ai.executionRecords.viewReport")
          }}</el-button>
    ...
    <!-- 报告对话框 -->
    <AIExecutionReport v-model="showReportDialog" :record-id="reportRecordId" />
  </div>
</template>
...
const batchDeleteRecords = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      t("uiAutomation.ai.executionRecords.messages.batchDeleteConfirm", {
    ...
    ElMessage.success(
      t("uiAutomation.ai.executionRecords.messages.deleteSuccess"),
    );
    ...
    if (error !== "cancel") {
      ...
      ElMessage.error(
        t("uiAutomation.ai.executionRecords.messages.batchDeleteFailed"),
      );
    }
  } finally {
...
const startPolling = () => {
  pollTimer = setInterval(() => {
    ...
    if (
      ...
      const hasActiveTasks = records.value.some(
