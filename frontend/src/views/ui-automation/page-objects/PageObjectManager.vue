<template>
  <div class="page-object-manager">
          ...
          >
            <div class="item-header">
              ...
              <div class="item-actions">
                <el-button
                  size="small"
                  text
                  @click.stop="editPageObject(pageObject)"
                >
                ...
                <el-button
                  ...
                  text
                  type="danger"
                  @click.stop="deletePageObject(pageObject.id)"
                >
            ...
            <div class="item-meta">
              ...
              <el-tag size="small" type="info"
                >{{ pageObject.elements_count || 0 }}
                {{ $t("uiAutomation.pageObject.elementsCount") }}</el-tag
              >
            </div>
      ...
      <div class="right-panel">
        <div v-if="selectedPageObject" class="page-object-detail">
          ...
          <el-tabs v-model="activeTab" type="border-card">
            <!-- 元素设计 -->
            <el-tab-pane
              :label="$t('uiAutomation.pageObject.elementsDesign')"
              name="design"
            >
              <div class="design-area">
                <div class="element-library">
                  ...
                  <div class="element-list">
                    ...
                    >
                      ...
                      <el-tag
                        size="small"
                        :type="getElementTypeTag(element.element_type)"
                      >
                ...
                <div class="design-canvas">
                  <h4>
                    {{ $t("uiAutomation.pageObject.pageObjectElements") }}
                  </h4>
                  <div
                  ...
                  >
                    <div
                      v-if="pageObjectElements.length === 0"
                      class="empty-canvas"
                    >
                      <el-empty
                        :description="
                          $t('uiAutomation.pageObject.dragElementHint')
                        "
                      />
                    </div>
                      ...
                      >
                        <div class="element-header">
                          <el-icon>
                            <component
                              :is="
                                getElementIcon(poElement.element.element_type)
                              "
                            />
                          </el-icon>
                          <span class="method-name">{{
                            poElement.method_name
                          }}</span>
                          <div class="element-actions">
                            <el-button
                              size="small"
                              text
                              @click="editCanvasElement(poElement)"
                            >
                              <el-icon><Edit /></el-icon>
                            </el-button>
                            <el-button
                              ...
                              text
                              type="danger"
                              @click="removeCanvasElement(poElement.id)"
                            >
                              <el-icon><Delete /></el-icon>
                            </el-button>
                          </div>
                        ...
                        <div class="element-meta">
                          <span class="element-name">{{
                            poElement.element.name
                          }}</span>
                          <el-tag
                            size="small"
                            :type="
                              poElement.is_property ? 'success' : 'primary'
                            "
                          >
                            {{
                              poElement.is_property
                                ? $t("uiAutomation.pageObject.property")
                                : $t("uiAutomation.pageObject.method")
                            }}
                          </el-tag>
                        </div>
                      </div>
            ...
            <!-- 代码预览 -->
            <el-tab-pane
              :label="$t('uiAutomation.pageObject.codePreview')"
              name="code"
            >
            ...
            <!-- 属性配置 -->
            <el-tab-pane
              :label="$t('uiAutomation.pageObject.propertiesConfig')"
              name="properties"
            >
              <div class="properties-panel" v-if="selectedCanvasElement">
                <h4>
                  {{ $t("uiAutomation.pageObject.elementPropertiesConfig") }}
                </h4>
                <el-form :model="selectedCanvasElement" label-width="100px">
                  <el-form-item
                    :label="$t('uiAutomation.pageObject.methodName')"
                  >
                    <el-input
                      v-model="selectedCanvasElement.method_name"
                      @change="updateCanvasElement"
                    />
                  </el-form-item>
                  <el-form-item :label="$t('uiAutomation.pageObject.type')">
                    <el-radio-group
                      v-model="selectedCanvasElement.is_property"
                      @change="updateCanvasElement"
                    >
                      <el-radio :label="true">{{
                        $t("uiAutomation.pageObject.property")
                  ...
                  <el-form-item :label="$t('uiAutomation.pageObject.order')">
                    <el-input-number
                      v-model="selectedCanvasElement.order"
                      @change="updateCanvasElement"
                    />
                  </el-form-item>
                </el-form>
                ...
                <el-descriptions :column="1" border>
                  <el-descriptions-item
                    :label="$t('uiAutomation.pageObject.elementName')"
                  >
                    {{ selectedCanvasElement.element.name }}
                  </el-descriptions-item>
                  <el-descriptions-item
                    :label="$t('uiAutomation.pageObject.elementType')"
                  >
                    {{
                      getElementTypeText(
                        selectedCanvasElement.element.element_type,
                      )
                    }}
                  </el-descriptions-item>
                  <el-descriptions-item
                    :label="$t('uiAutomation.pageObject.locator')"
                  >
              ...
              <div v-else class="no-selection">
                <el-empty
                  :description="
                    $t('uiAutomation.pageObject.selectElementToConfig')
                  "
                />
              </div>
            </el-tab-pane>
        ...
        <div v-else class="no-page-object">
          <el-empty
            :description="$t('uiAutomation.pageObject.selectPageObjectToEdit')"
          />
        </div>
      </div>
    </div>
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.pageObject.createPageObject')"
      width="600px"
    ...
    >
      ...
      >
        <el-form-item
          :label="$t('uiAutomation.pageObject.pageObjectName')"
          prop="name"
        >
          <el-input
            v-model="createForm.name"
            :placeholder="$t('uiAutomation.pageObject.rules.nameRequired')"
          />
        </el-form-item>
        <el-form-item
          :label="$t('uiAutomation.pageObject.className')"
          prop="class_name"
        >
          <el-input
            v-model="createForm.class_name"
            :placeholder="$t('uiAutomation.pageObject.rules.classNameRequired')"
          />
        </el-form-item>
        <el-form-item :label="$t('uiAutomation.pageObject.urlPattern')">
          <el-input
            v-model="createForm.url_pattern"
            :placeholder="$t('uiAutomation.pageObject.urlPatternPlaceholder')"
          />
        </el-form-item>
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.pageObject.addElementToPageObject')"
      width="600px"
    >
      <el-form
        ref="addElementFormRef"
        :model="addElementForm"
        :rules="addElementRules"
        label-width="120px"
      >
        <el-form-item
          :label="$t('uiAutomation.pageObject.selectElement')"
          prop="element_id"
        >
          <el-select
            v-model="addElementForm.element_id"
            :placeholder="$t('uiAutomation.pageObject.selectElement')"
            filterable
          >
            <el-option
        ...
        >
          <el-input
            v-model="addElementForm.method_name"
            :placeholder="
              $t('uiAutomation.pageObject.rules.methodNameRequired')
            "
          />
        </el-form-item>
        <el-form-item :label="$t('uiAutomation.pageObject.type')">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="showAddElementDialog = false">{{
            $t("uiAutomation.common.cancel")
          }}</el-button>
...
const formRules = computed(() => ({
  name: [
    {
      required: true,
      message: t("uiAutomation.pageObject.rules.nameRequired"),
      trigger: "blur",
    },
  ],
  class_name: [
    {
      required: true,
      message: t("uiAutomation.pageObject.rules.classNameRequired"),
      trigger: "blur",
    },
    {
      pattern: /^[A-Z][a-zA-Z0-9]*$/,
      message: t("uiAutomation.pageObject.rules.classNamePattern"),
      trigger: "blur",
    },
  ],
}));
...
const addElementRules = computed(() => ({
  element_id: [
    {
      required: true,
      message: t("uiAutomation.pageObject.rules.elementRequired"),
      trigger: "change",
    },
  ],
  method_name: [
    {
      required: true,
      message: t("uiAutomation.pageObject.rules.methodNameRequired"),
      trigger: "blur",
    },
    {
      pattern: /^[a-zA-Z_][a-zA-Z0-9_]*$/,
      message: t("uiAutomation.pageObject.rules.methodNamePattern"),
      trigger: "blur",
    },
  ],
}));
...
const filteredPageObjects = computed(() => {
  ...
  return pageObjects.value.filter(
    (po) =>
...
const filteredElements = computed(() => {
  ...
  return availableElements.value.filter(
    (element) =>
...
const handleDrop = (event) => {
  ...
  if (!selectedPageObject.value) {
    ElMessage.warning(
      t("uiAutomation.pageObject.messages.selectPageObjectFirst"),
    );
    return;
  }
  ...
  // 检查元素是否已经存在
  const exists = pageObjectElements.value.some(
    (pe) => pe.element.id === elementData.id,
  );
  if (exists) {
    ElMessage.warning(
      t("uiAutomation.pageObject.messages.elementAlreadyExists"),
    );
    return;
  }

  // 自动生成方法名称
  const methodName = elementData.name
    .replace(/[\s\-]/g, "")
...
const updateCanvasElement = async () => {
  ...
  try {
    ...
    ElMessage.success(
      t("uiAutomation.pageObject.messages.updateConfigSuccess"),
    );
  } catch (error) {
...
const removeCanvasElement = async (elementId) => {
  try {
    ...
    ElMessage.success(
      t("uiAutomation.pageObject.messages.removeElementSuccess"),
    );
  } catch (error) {
...
const generateCode = async () => {
  ...
  try {
    ...
    ElMessage.success(
      t("uiAutomation.pageObject.messages.generateCodeSuccess"),
    );
  } catch (error) {
...
const deletePageObject = async (id) => {
  try {
    ...
    ElMessage.success(
      t("uiAutomation.pageObject.messages.deletePageObjectSuccess"),
    );
  } catch (error) {
    if (error === "cancel") return;
    console.error("删除页面对象失败:", error);
    ElMessage.error(
      t("uiAutomation.pageObject.messages.deletePageObjectFailed"),
    );
  }
};
...
const handleCreate = async () => {
  ...
  try {
    ...
    ElMessage.success(
      t("uiAutomation.pageObject.messages.createPageObjectSuccess"),
    );
    ...
    ElMessage.error(
      t("uiAutomation.pageObject.messages.createPageObjectFailed"),
    );
  }
};
