<template>
  <div class="element-manager">
        ...
        <div class="sidebar-header">
          ...
          <div class="header-actions">
            <el-button
              ...
              :title="$t('uiAutomation.element.createPage')"
            >
            ...
            <el-button
              type="success"
              size="small"
              @click="createEmptyElement"
              :title="$t('uiAutomation.element.addElement')"
            >
          ...
          >
            <template #default="{ node, data }">
              <div class="tree-node">
                ...
                <!-- 页面名称编辑 -->
                <div
                  v-if="data.type === 'page' && editingNodeId === data.id"
                  class="node-edit"
                >
                  <el-input
                ...
                <span
                  ...
                  :class="data.element_type?.toLowerCase()"
                >
      ...
      <div class="main-content">
        <div v-if="!selectedElement" class="empty-state">
          <el-empty :description="$t('uiAutomation.element.emptyElementTip')">
            <el-button type="primary" @click="createEmptyElement">{{
              $t("uiAutomation.element.createNewElement")
            }}</el-button>
          </el-empty>
        </div>
        ...
        <div v-else class="element-detail">
          ...
          <div class="element-header">
            <div class="element-info">
              ...
              />
              <el-select
                v-model="selectedElement.element_type"
                :placeholder="$t('uiAutomation.element.elementType')"
                size="small"
                style="width: 120px"
              >
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.button')"
                  value="BUTTON"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.input')"
                  value="INPUT"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.link')"
                  value="LINK"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.dropdown')"
                  value="DROPDOWN"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.checkbox')"
                  value="CHECKBOX"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.radio')"
                  value="RADIO"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.text')"
                  value="TEXT"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.image')"
                  value="IMAGE"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.table')"
                  value="TABLE"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.form')"
                  value="FORM"
                />
                <el-option
                  :label="$t('uiAutomation.element.elementTypes.modal')"
                  value="MODAL"
                />
              </el-select>
              <el-button
          ...
          <div class="element-form">
            ...
            >
              <el-row :gutter="20">
                ...
                <el-col :span="12">
                  <el-form-item
                    :label="$t('uiAutomation.element.componentName')"
                  >
                    <el-input
                      v-model="selectedElement.component_name"
                      :placeholder="
                        $t('uiAutomation.element.componentNamePlaceholder')
                      "
                    />
                  </el-form-item>
                </el-col>
              ...
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item
                    :label="$t('uiAutomation.element.locatorStrategy')"
                    prop="locator_strategy_id"
                  >
                    <el-select
                    ...
                    <div style="font-size: 10px; color: #666; margin-top: 2px">
                      {{ $t("uiAutomation.element.debugInfo") }}:
                      {{ $t("uiAutomation.element.currentValue") }}={{
                        selectedElement.locator_strategy_id
                      }}
                      ({{ typeof selectedElement.locator_strategy_id }})
                    </div>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item
                    :label="
                      $t('uiAutomation.element.waitTimeout') +
                      '(' +
                      $t('uiAutomation.element.waitTimeoutUnit') +
                      ')'
                    "
                  >
                    <el-input-number
                      v-model="selectedElement.wait_timeout"
                      :min="1"
                      :max="60"
                      style="width: 100%"
                    />
                ...
                <el-col :span="12">
                  <el-form-item :label="$t('uiAutomation.element.forceAction')">
                    <el-switch
                      ...
                      :active-text="
                        $t('uiAutomation.element.forceActionEnabled')
                      "
                      :inactive-text="
                        $t('uiAutomation.element.forceActionDisabled')
                      "
                    />
              ...
              <el-form-item
                :label="$t('uiAutomation.element.locatorExpression')"
                prop="locator_value"
              >
                <el-input
                  v-model="selectedElement.locator_value"
                  :placeholder="
                    $t('uiAutomation.element.locatorExpressionPlaceholder')
                  "
                />
              ...
              <el-form-item :label="$t('uiAutomation.common.description')">
                <el-input
                  ...
                  :rows="3"
                  :placeholder="
                    $t('uiAutomation.element.descriptionPlaceholder')
                  "
                />
              </el-form-item>
            </el-form>
    ...
    <!-- 创建页面对话框 -->
    <el-dialog
      v-model="showCreatePageDialog"
      :title="$t('uiAutomation.element.createPageTitle')"
      width="500px"
      ...
      >
        <el-form-item :label="$t('uiAutomation.element.pageName')" prop="name">
          <el-input
            ...
            :placeholder="$t('uiAutomation.element.pageNamePlaceholder')"
          />
        </el-form-item>
        ...
        >
          <el-input
            ...
            type="textarea"
            :rows="3"
            :placeholder="$t('uiAutomation.element.descriptionPlaceholder')"
          />
        </el-form-item>
      </el-form>

      <template #footer>
        <el-button @click="showCreatePageDialog = false">{{
    ...
    <el-dialog
      ...
      :title="$t('uiAutomation.element.editPageTitle')"
      width="500px"
      :close-on-click-modal="false"
    >
      <el-form
      ...
      >
        ...
        >
          <el-input
            v-model="editPageForm.description"
            type="textarea"
            :rows="3"
            :placeholder="$t('uiAutomation.element.descriptionPlaceholder')"
          />
      ...
      <template #footer>
        <el-button @click="showEditPageDialog = false">{{
          $t("uiAutomation.common.cancel")
        }}</el-button>
...
const pageRules = computed(() => ({
  name: [
    {
      required: true,
      message: t("uiAutomation.element.rules.pageNameRequired"),
      trigger: "blur",
    },
  ],
}));
...
const exposeToWindow = () => {
  if (typeof window !== "undefined") {
    window.ELEMENTS_DEBUG = {
      ...
      selectedElement,
      loadElementTree,
      treeRef: typeof treeRef.value !== "undefined" ? treeRef : null,
      expandedKeys,
      pages,
      $vm: {
        // 当前组件实例
  ...
  if (typeof window !== "undefined") {
    ...
    console.log(
      ...
      treeData.value.map((p) => ({
        ...
        elementChildren:
          p.children?.filter((c) => c.type === "element").map((e) => e.name) ||
          [],
      })),
    );
...
const loadPageTree = async () => {
  ...
  try {
    const response = await getElementGroupTree({
...
const loadElementTree = async () => {
  ...
  try {
    ...
    console.log(
      "页面树原始数据:",
      JSON.parse(JSON.stringify(pageTreeResponse.data || []), null, 2),
    );
    ...
    const attachElementsToPages = (pages) => {
      pages.forEach((page) => {
        ...
        console.log(
          `页面 ${page.name} (ID: ${page.id}) 找到 ${pageElements.length} 个关联元素`,
          pageElements,
        );
        ...
        page.children = page.children
          ...
          : [...elementNodes];
        console.log(
          `页面 ${page.name} 现在有 ${page.children.filter((c) => c.type === "element").length} 个子元素`,
        );
    ...
    if (typeof window !== "undefined") {
      ...
      console.log(
        "当前treeData结构:",
        JSON.parse(JSON.stringify(treeData.value)).map((p) => ({
          name: p.name,
          id: p.id,
  ...
  try {
    ...
    if (selectedElement.value.id) {
      ...
      if (selectedElement.value.page) {
        ...
        const pageId = findPageIdByName(
          treeData.value,
          selectedElement.value.page,
        );
        if (pageId) {
      ...
      console.log(
        "locator_strategy_id值:",
        selectedElement.value.locator_strategy_id,
        "类型:",
        typeof selectedElement.value.locator_strategy_id,
      );
      console.log(
        "locator_strategy对象:",
        selectedElement.value.locator_strategy,
      );
      console.log("当前locatorStrategies:", locatorStrategies.value);
      console.log(
        "locatorStrategies中是否包含id=" +
          selectedElement.value.locator_strategy_id +
          ":",
        locatorStrategies.value.find(
      ...
      if (selectedElement.value.page) {
        ...
        const findPageIdByName = (nodes, pageName) => {
          ...
          for (const node of nodes) {
            console.log(
              `检查节点: ${node.name} (ID: ${node.id}, type: ${node.type})`,
            );
            if (node.type === "page" && node.name === pageName) {
...
const validateElement = async () => {
  ...
  try {
    ...
    if (result.is_valid) {
      ...
      ElMessage.error(
        `${t("uiAutomation.element.messages.validateFailed")}: ${result.validation_message}`,
      );
    }
  } catch (error) {
...
const editNode = async () => {
  ...
  if (rightClickedNode.value.type === "page") {
    ...
    try {
      ...
      selectedElement.value = response.data;
      console.log(
        "Set selected element for editing via API:",
        selectedElement.value,
      );
...
const deleteNode = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      t("uiAutomation.element.messages.confirmDeleteNode", {
    ...
    if (rightClickedNode.value.type === "page") {
      // 删除页面（分组）
      console.log(
        "Calling deleteElementGroup with id:",
        rightClickedNode.value.id,
      );
      ...
      if (
        selectedElement.value &&
        selectedElement.value.id === rightClickedNode.value.id
      ) {
        selectedElement.value = null;
      }
    }
