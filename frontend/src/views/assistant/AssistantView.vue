        ...
        <div class="session-scroll-area">
          ...
          >
            ...
            <div class="session-actions" @click.stop>
              <el-popconfirm
                :title="$t('assistant.deleteSessionConfirm')"
      ...
      <div class="user-profile">
        <el-dropdown trigger="click" @command="handleCommand">
          <div class="user-info">
            ...
            <span class="username">{{
              userStore.user?.username || $t("assistant.user")
            }}</span>
        ...
        <div class="welcome-content">
          ...
          <div class="suggestion-chips">
            <div
              class="chip"
              @click="
                useSuggestion($t('assistant.suggestions.apiTestQuestion'))
              "
            >
              {{ $t("assistant.suggestions.apiTest") }}
            </div>
            <div
              class="chip"
              @click="
                useSuggestion(
                  $t('assistant.suggestions.performancePlanQuestion'),
                )
              "
            >
              {{ $t("assistant.suggestions.performancePlan") }}
            </div>
            <div
              class="chip"
              @click="
                useSuggestion($t('assistant.suggestions.testTheoryQuestion'))
              "
            >
              {{ $t("assistant.suggestions.testTheory") }}
            </div>
            <div
              class="chip"
              @click="
                useSuggestion(
                  $t('assistant.suggestions.automationDebugQuestion'),
                )
              "
            >
              {{ $t("assistant.suggestions.automationDebug") }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else class="chat-screen">
        <div class="chat-header">
          <span class="chat-title">{{
            currentSession?.title || $t("assistant.newChat")
          }}</span>
          <span class="chat-time" v-if="currentSession">{{
...
const handleLogout = () => {
  ElMessageBox.confirm(
    t("assistant.logoutConfirm"),
    t("assistant.logoutTitle"),
    {
...
const historySessionsDescending = computed(() => {
  return [...historySessions.value].sort(
...
const isNewChatMode = computed(() => {
  ...
  return (
    ...
    (!currentSession.value.id && messages.value.length === 0)
  );
});
...
const sendMessage = async () => {
  ...
  try {
    ...
    if (!isFirstMessage) {
      const index = historySessions.value.findIndex(
        (s) => s.id === currentSession.value.id,
      );
      if (index !== -1) {
        historySessions.value[index] = {
          ...currentSession.value,
    ...
    messages.value.pop(); // 移除思考中
    ElMessage.error(
      error.response?.data?.error || t("assistant.messages.sendFailed"),
    );
  } finally {
    sending.value = false;
    scrollToBottom();
