  ...
  <div class="scheduled-tasks">
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-select
            ...
            :placeholder="$t('apiTesting.scheduledTask.taskType')"
            clearable
          >
            <el-option
              :label="$t('apiTesting.scheduledTask.taskTypes.testSuite')"
              value="TEST_SUITE"
            />
            <el-option
              :label="$t('apiTesting.scheduledTask.taskTypes.apiRequest')"
              value="API_REQUEST"
            />
          </el-select>
        </el-col>
        <el-col :span="6">
          <el-select
            ...
            :placeholder="$t('apiTesting.scheduledTask.triggerType')"
            clearable
          >
            <el-option
              :label="$t('apiTesting.scheduledTask.triggerTypes.cron')"
              value="CRON"
            />
            <el-option
              :label="$t('apiTesting.scheduledTask.triggerTypes.interval')"
              value="INTERVAL"
            />
            <el-option
              :label="$t('apiTesting.scheduledTask.triggerTypes.once')"
              value="ONCE"
            />
          </el-select>
        </el-col>
        <el-col :span="6">
          <el-select
            ...
            :placeholder="$t('apiTesting.scheduledTask.taskStatus')"
            clearable
          >
            <el-option
              :label="$t('apiTesting.scheduledTask.status.active')"
              value="ACTIVE"
            />
            <el-option
              :label="$t('apiTesting.scheduledTask.status.paused')"
              value="PAUSED"
            />
            <el-option
              :label="$t('apiTesting.scheduledTask.status.completed')"
              value="COMPLETED"
            />
            <el-option
              :label="$t('apiTesting.scheduledTask.status.failed')"
              value="FAILED"
            />
          </el-select>
        </el-col>
    ...
    <div class="task-list">
      <el-table :data="tasks" v-loading="loading">
        <el-table-column
          prop="name"
          :label="$t('apiTesting.scheduledTask.taskName')"
          min-width="200"
        />
        <el-table-column
          prop="task_type"
          :label="$t('apiTesting.scheduledTask.taskType')"
          width="120"
        >
            ...
            >
              {{
                scope.row.task_type === "TEST_SUITE"
                  ? $t("apiTesting.scheduledTask.taskTypes.testSuiteShort")
                  : $t("apiTesting.scheduledTask.taskTypes.apiRequestShort")
              }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="trigger_type"
          :label="$t('apiTesting.scheduledTask.triggerType')"
          width="120"
        >
          <template #default="scope">
            <el-tag>
        ...
        <el-table-column
          ...
          :label="$t('apiTesting.scheduledTask.notificationType')"
          width="120"
        >
          <template #default="scope">
            <el-tag
              :type="
                getNotificationTypeTag(scope.row.notification_type_display)
              "
              size="small"
            >
        ...
        <el-table-column
          prop="visibility"
          :label="$t('apiTesting.common.visibility')"
          width="110"
        >
          <template #default="scope">
            <el-tag :type="scope.row.visibility === 'all' ? 'success' : 'info'" size="small">
              {{ scope.row.visibility === 'all' ? $t('apiTesting.common.visibleAll') : $t('apiTesting.common.visibleSelf') }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="next_run_time"
          :label="$t('apiTesting.scheduledTask.nextRunTime')"
          width="180"
        >
          <template #default="scope">
        ...
        <el-table-column
          prop="last_run_time"
          :label="$t('apiTesting.scheduledTask.lastRunTime')"
          width="180"
        >
          <template #default="scope">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item
                    ...
                    >{{
                      $t("apiTesting.scheduledTask.pause")
                    }}</el-dropdown-item
                  >
                  <el-dropdown-item
                    command="activate"
                    ...
                    >{{
                      $t("apiTesting.scheduledTask.activate")
                    }}</el-dropdown-item
                  >
                  <el-dropdown-item command="logs">{{
                    $t("apiTesting.scheduledTask.executionLogs")
                  }}</el-dropdown-item>
    ...
    <el-dialog
      ...
      :title="
        editingTask
          ? $t('apiTesting.scheduledTask.editTask')
          : $t('apiTesting.scheduledTask.createTask')
      "
      width="800px"
      ...
      <el-form :model="taskForm" label-width="120px">
        <el-form-item :label="$t('apiTesting.scheduledTask.taskName')" required>
          <el-input
            ...
            :placeholder="$t('apiTesting.scheduledTask.inputTaskName')"
          />
        </el-form-item>
        ...
        <el-form-item :label="$t('apiTesting.scheduledTask.taskDescription')">
          <el-input
            ...
            type="textarea"
            :placeholder="$t('apiTesting.scheduledTask.inputTaskDesc')"
          />
        </el-form-item>

        <el-form-item :label="$t('apiTesting.scheduledTask.taskType')" required>
          <el-radio-group v-model="taskForm.task_type">
            <el-radio label="TEST_SUITE">{{
              $t("apiTesting.scheduledTask.taskTypes.testSuite")
            }}</el-radio>
            <el-radio label="API_REQUEST">{{
              $t("apiTesting.scheduledTask.taskTypes.apiRequest")
            }}</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        >
          <el-radio-group v-model="taskForm.trigger_type">
            <el-radio label="CRON">{{
              $t("apiTesting.scheduledTask.triggerTypes.cron")
            }}</el-radio>
            <el-radio label="INTERVAL">{{
              $t("apiTesting.scheduledTask.triggerTypes.interval")
            }}</el-radio>
            <el-radio label="ONCE">{{
              $t("apiTesting.scheduledTask.triggerTypes.once")
            }}</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item
          ...
          :label="$t('apiTesting.scheduledTask.cronExpression')"
          required
        >
          <el-input
            ...
            <el-tooltip raw-content placement="top">
              <template #content>
                <div style="line-height: 1.6; text-align: left">
                  <div>
                    {{ $t("apiTesting.scheduledTask.cronHelp.format") }}
                  </div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.minute") }}
                  </div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.hour") }}
                  </div>
                  <div>• {{ $t("apiTesting.scheduledTask.cronHelp.day") }}</div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.month") }}
                  </div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.week") }}
                  </div>
                  <div style="margin-top: 8px">
                    {{ $t("apiTesting.scheduledTask.cronHelp.examples") }}
                  </div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.daily") }}
                  </div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.hourly") }}
                  </div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.weekly") }}
                  </div>
                  <div>
                    • {{ $t("apiTesting.scheduledTask.cronHelp.monthly") }}
                  </div>
                </div>
              </template>
              <span style="cursor: pointer; color: #409eff">{{
                $t("apiTesting.scheduledTask.cronHelpLink")
              }}</span>
            </el-tooltip>
          </div>
        </el-form-item>
        ...
        <el-form-item
          ...
          :label="$t('apiTesting.scheduledTask.intervalTime')"
          required
        >
          <el-input-number
        ...
        <el-form-item
          ...
          :label="$t('apiTesting.scheduledTask.executeTime')"
          required
        >
          <el-date-picker
        ...
        <el-form-item
          ...
          :label="$t('apiTesting.automation.testSuite')"
          required
        >
          <el-select
            v-model="taskForm.test_suite"
            :placeholder="$t('apiTesting.scheduledTask.selectTestSuite')"
          >
            <el-option
        ...
        <el-form-item
          v-if="taskForm.task_type === 'API_REQUEST'"
          :label="$t('apiTesting.scheduledTask.apiRequest')"
          required
        >
          <el-select
            v-model="taskForm.api_request"
            :placeholder="$t('apiTesting.scheduledTask.selectApiRequest')"
          >
            <el-option
        ...
        <el-form-item
          :label="$t('apiTesting.scheduledTask.executeEnvironment')"
        >
          <el-select
            v-model="taskForm.environment"
            :placeholder="$t('apiTesting.scheduledTask.selectEnvironment')"
          >
            <el-option
        ...
        <el-form-item
          :label="$t('apiTesting.scheduledTask.notificationSettings')"
        >
          <el-checkbox v-model="taskForm.notify_on_success">{{
            $t("apiTesting.scheduledTask.notifyOnSuccess")
          }}</el-checkbox>
          <el-checkbox v-model="taskForm.notify_on_failure">{{
            $t("apiTesting.scheduledTask.notifyOnFailure")
          }}</el-checkbox>
        </el-form-item>
        ...
        >
          <el-select
            ...
            :placeholder="$t('apiTesting.scheduledTask.selectNotificationType')"
          >
            <el-option
              :label="$t('apiTesting.notification.types.email')"
              value="email"
            />
            <el-option
              :label="$t('apiTesting.notification.types.webhook')"
              value="webhook"
            />
            <el-option
              :label="$t('apiTesting.notification.types.both')"
              value="both"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item
          ...
          "
          :label="$t('apiTesting.scheduledTask.notifyEmails')"
        >
          <el-select
    ...
    <el-dialog
      ...
      :title="$t('apiTesting.scheduledTask.executionLogs')"
      width="1000px"
    >
      <el-table :data="executionLogs" v-loading="logsLoading">
        <el-table-column
          prop="start_time"
          :label="$t('apiTesting.scheduledTask.startTime')"
          width="180"
        >
          <template #default="scope">
            <div class="time-cell">
        ...
        <el-table-column
          prop="end_time"
          :label="$t('apiTesting.scheduledTask.endTime')"
          width="180"
        >
          <template #default="scope">
            <div class="time-cell">
            ...
            >
              {{
                scope.row.status === "COMPLETED"
                  ? $t("apiTesting.common.success")
                  : $t("apiTesting.common.failed")
              }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="error_message"
          :label="$t('apiTesting.scheduledTask.errorMessage')"
          width="300"
...
const loadApiRequests = async () => {
  try {
    const response = await getApiRequests({ page_size: 1000 });
  ...
  try {
    ...
    ElMessage.error(
      error.response?.data?.error ||
        ...
        (editingTask.value
          ? t("apiTesting.messages.error.updateTaskFailed")
          : t("apiTesting.messages.error.createTaskFailed")),
    );
  } finally {
