<template>
  <div class="project-management">
    ...
    <el-table :data="projects" v-loading="loading" style="width: 100%">
      <el-table-column
        prop="name"
        :label="$t('apiTesting.project.projectName')"
        min-width="200"
      />
      <el-table-column
        prop="project_type"
        :label="$t('apiTesting.project.projectType')"
        width="120"
      >
        <template #default="scope">
          <el-tag
            :type="scope.row.project_type === 'HTTP' ? 'primary' : 'success'"
          >
      ...
      <el-table-column
        prop="status"
        :label="$t('apiTesting.project.projectStatus')"
        width="120"
      >
        <template #default="scope">
      ...
      <el-table-column
        prop="owner.username"
        :label="$t('apiTesting.project.owner')"
        width="150"
      />
      <el-table-column
        prop="start_date"
        :label="$t('apiTesting.project.startDate')"
        width="120"
      />
      <el-table-column
        prop="end_date"
        :label="$t('apiTesting.project.endDate')"
        width="120"
      />
      <el-table-column
        prop="created_at"
        :label="$t('apiTesting.project.createdAt')"
        width="180"
      >
        <template #default="scope">
      ...
      <el-table-column :label="$t('apiTesting.common.operation')" width="200">
        <template #default="scope">
          <el-button link type="primary" @click="editProject(scope.row)">{{
            $t("apiTesting.common.edit")
          }}</el-button>
          <el-button link type="primary" @click="viewProject(scope.row)">{{
            $t("apiTesting.common.view")
          }}</el-button>
          <el-button link type="danger" @click="deleteProject(scope.row)">{{
    ...
    <el-dialog
      ...
      :title="
        editingProject
          ? $t('apiTesting.project.editProject')
          : $t('apiTesting.project.createProject')
      "
      width="600px"
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item
          :label="$t('apiTesting.project.projectDescription')"
          prop="description"
        >
          <el-input
        ...
        >
          ...
          >
            <el-option
              :label="$t('apiTesting.project.status.notStarted')"
              value="NOT_STARTED"
            />
            <el-option
              :label="$t('apiTesting.project.status.inProgress')"
              value="IN_PROGRESS"
            />
            <el-option
              :label="$t('apiTesting.project.status.completed')"
              value="COMPLETED"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item :label="$t('apiTesting.project.owner')" prop="owner">
          <el-select
            v-model="form.owner"
            :placeholder="$t('apiTesting.project.selectOwner')"
            filterable
          >
            <el-option
              v-for="user in users"
        ...
        <el-form-item
          :label="$t('apiTesting.project.teamMembers')"
          prop="member_ids"
        >
          <el-select
    ...
    >
      <el-descriptions :column="1" border>
        <el-descriptions-item :label="$t('apiTesting.project.projectName')">{{
          viewedProject?.name
        }}</el-descriptions-item>
        <el-descriptions-item
          :label="$t('apiTesting.project.projectDescription')"
          >{{
            viewedProject?.description || $t("apiTesting.project.none")
          }}</el-descriptions-item
        >
        <el-descriptions-item :label="$t('apiTesting.project.projectType')">
          <el-tag
            :type="
              viewedProject?.project_type === 'HTTP' ? 'primary' : 'success'
            "
          >
        ...
        <el-descriptions-item :label="$t('apiTesting.project.owner')">{{
          viewedProject?.owner?.username
        }}</el-descriptions-item>
        ...
        <el-descriptions-item :label="$t('apiTesting.project.startDate')">{{
          viewedProject?.start_date || $t("apiTesting.project.notSet")
        }}</el-descriptions-item>
        <el-descriptions-item :label="$t('apiTesting.project.endDate')">{{
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="editProject(viewedProject)">{{
          $t("apiTesting.common.edit")
        }}</el-button>
      </template>
    </el-dialog>
...
const rules = computed(() => ({
  name: [
    {
      required: true,
      message: t("apiTesting.project.inputProjectName"),
      trigger: "blur",
    },
  ],
  project_type: [
    {
      required: true,
      message: t("apiTesting.common.pleaseSelect"),
      trigger: "change",
    },
  ],
  status: [
    {
      required: true,
      message: t("apiTesting.project.selectStatus"),
      trigger: "change",
    },
  ],
  owner: [
    {
      required: true,
      message: t("apiTesting.project.selectOwner"),
      trigger: "change",
    },
  ],
}));
