<template>
  <div class="interface-management">
        ...
        <div class="sidebar-header">
          ...
          <div class="header-actions">
            ...
            <el-button
              type="warning"
              size="small"
              @click="openSwaggerImportDialog"
              title="从 Swagger 导入接口"
            >
        ...
        >
          ...
          >
            <template #default="{ node, data }">
              <div class="tree-node" :class="{ 'is-multi-selected': data.type === 'request' && selectedRequests.some(r => r.id === data.id) }" @click="onNodeClick(data, $event)">
                ...
                <!-- 集合名称编辑 -->
                <div
                  v-if="data.type === 'collection' && editingNodeId === data.id"
                  class="node-edit"
                >
                  <el-input
                ...
                <span
                  v-if="
                    data.type === 'request' && data.request_type !== 'WEBSOCKET'
                  "
                  class="method-tag"
          ...
          <div v-if="filteredCollections.length > 0" class="search-results">
            <div class="search-results-header">
              <span>{{
                $t("apiTesting.interface.searchResults", {
            ...
            <div class="search-results-list">
              ...
              >
                ...
                <el-tag
                  v-if="item.matchType === 'name'"
                  type="primary"
                  size="small"
                  >{{ $t("apiTesting.interface.name") }}</el-tag
                >
                <el-tag
                  v-else-if="item.matchType === 'method'"
                  type="warning"
                  size="small"
                  >{{ $t("apiTesting.interface.method") }}</el-tag
                >
                <el-tag v-else type="success" size="small">{{
                  $t("apiTesting.interface.url")
                }}</el-tag>
              </div>
            </div>
          </div>
      ...
      <div class="main-content">
        <div v-if="!selectedRequest" class="empty-state">
          <el-empty :description="$t('apiTesting.interface.selectInterface')">
            <el-button type="primary" @click="createEmptyRequest">{{
              $t("apiTesting.interface.createNewInterface")
            }}</el-button>
          </el-empty>
        </div>
        ...
        <div v-else class="request-detail">
          ...
          <div class="request-header">
            <div class="request-line">
              ...
              >
                <template #prepend>
                  <el-select
                    ...
                    :placeholder="$t('apiTesting.interface.environment')"
                    class="env-select"
                  >
                    <el-option
                      :label="$t('apiTesting.common.noEnvironment')"
                      :value="null"
                    />
                    <el-option
              ...
              <el-button
                v-if="
                  selectedRequest &&
                  selectedRequest.request_type === 'WEBSOCKET'
                "
                :type="
                ...
                class="send-button"
              >
                <span v-if="websocketConnectionStatus === 'disconnected'">{{
                  $t("apiTesting.interface.connect")
                }}</span>
                <span v-else-if="websocketConnectionStatus === 'connecting'">{{
                  $t("apiTesting.interface.connecting")
                }}</span>
            ...
            <div class="request-meta">
              ...
              />
              <el-select
                v-model="selectedRequest.visibility"
                size="small"
                style="width: 130px; flex-shrink: 0"
                :placeholder="$t('apiTesting.common.visibility')"
              >
                <el-option value="all" :label="$t('apiTesting.common.visibleAll')" />
                <el-option value="private" :label="$t('apiTesting.common.visibleSelf')" />
              </el-select>
              <el-tooltip
                :content="selectedRequest.skip_auth ? $t('apiTesting.interface.skipAuthOn') : $t('apiTesting.interface.skipAuthOff')"
                placement="top"
              >
                <div class="skip-auth-toggle" :class="{ active: selectedRequest.skip_auth }">
                  <el-switch
                    v-model="selectedRequest.skip_auth"
                    size="small"
                    :active-color="'#f56c6c'"
                  />
                  <span class="skip-auth-label">{{ $t('apiTesting.interface.skipAuth') }}</span>
                </div>
              </el-tooltip>
              <div class="action-buttons">
                ...
                <el-button
                  size="small"
                  :loading="cloning"
                  :disabled="!selectedRequest || !selectedRequest.id"
                  :title="$t('apiTesting.interface.cloneInterface')"
                  @click="cloneRequest"
                >
                  <el-icon><CopyDocument /></el-icon>
                  {{ $t("apiTesting.interface.clone") }}
                </el-button>
                <el-dropdown
                  split-button
                  type="default"
                ...
                >
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item @click="importCurl">{{
                        $t("apiTesting.interface.importCurl")
                      }}</el-dropdown-item>
                      <el-dropdown-item @click="exportRequest">{{
                        $t("apiTesting.interface.exportCurl")
                      }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
                ...
                >
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item @click="generateCode('javascript')"
                        >JavaScript</el-dropdown-item
                      >
                      <el-dropdown-item @click="generateCode('python')"
                        >Python</el-dropdown-item
                      >
                      <el-dropdown-item @click="generateCode('java')"
                        >Java</el-dropdown-item
                      >
                      <el-dropdown-item @click="generateCode('curl')"
                        >cURL</el-dropdown-item
                      >
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
          ...
          <el-tabs v-model="activeTab" class="request-tabs">
            ...
            <el-tab-pane label="Body" name="body" v-if="hasBody">
              <div class="body-container">
                <el-radio-group v-model="bodyType" @change="onBodyTypeChange">
                  ...
                  <el-radio value="x-www-form-urlencoded"
            ...
            <template
              v-if="
                !selectedRequest || selectedRequest.request_type !== 'WEBSOCKET'
              "
            >
                ...
                <div class="script-editor-container">
                  ...
                  <div class="script-buttons">
                    <el-button
                      ...
                      :icon="MagicStick"
                      @click="
                        openDataFactorySelectorForScript('pre_request_script')
                      "
                      :title="$t('apiTesting.interface.referDataFactory')"
                ...
                <div class="script-editor-container">
                  ...
                  <div class="script-buttons">
                    <el-button
                      ...
                      :icon="MagicStick"
                      @click="
                        openDataFactorySelectorForScript('post_request_script')
                      "
                      :title="$t('apiTesting.interface.referDataFactory')"
                      class="script-factory-btn"
                    >
              ...
              <el-tab-pane
                :label="$t('apiTesting.interface.assertions')"
                name="assertions"
              >
                <div class="assertions-editor">
                  ...
                  <div class="assertions-list">
                    ...
                    >
                      <div class="assertion-header">
                        <el-input
                          ...
                          :placeholder="
                            $t('apiTesting.interface.assertionName')
                          "
                          size="small"
                      ...
                      <div class="assertion-config">
                        <el-select
                          ...
                          :placeholder="
                            $t('apiTesting.interface.selectAssertionType')
                          "
                          size="small"
                        ...
                        >
                          <el-option
                            :label="
                              $t(
                                'apiTesting.interface.assertionTypes.statusCode',
                              )
                            "
                            value="status_code"
                          />
                          <el-option
                            :label="
                              $t(
                                'apiTesting.interface.assertionTypes.responseTime',
                              )
                            "
                            value="response_time"
                          />
                          <el-option
                            :label="
                              $t('apiTesting.interface.assertionTypes.contains')
                            "
                            value="contains"
                          />
                          <el-option
                            :label="
                              $t('apiTesting.interface.assertionTypes.jsonPath')
                            "
                            value="json_path"
                          />
                          <el-option
                            :label="
                              $t('apiTesting.interface.assertionTypes.header')
                            "
                            value="header"
                          />
                          <el-option
                            :label="
                              $t('apiTesting.interface.assertionTypes.equals')
                            "
                            value="equals"
                          />
                          <el-option
                            :label="
                              $t('apiTesting.interface.assertionTypes.mongoMatch')
                            "
                            value="mongo_match"
                          />
                        </el-select>
                        ...
                        <div class="assertion-params" v-if="assertion.type">
                          ...
                          <div v-if="assertion.type === 'status_code'">
                            <el-input-number
                              ...
                              size="small"
                              :placeholder="
                                $t('apiTesting.interface.expectedStatusCode')
                              "
                            />
                          </div>
                          ...
                          <div v-else-if="assertion.type === 'response_time'">
                            <el-input-number
                              ...
                              size="small"
                              :placeholder="
                                $t('apiTesting.interface.maxResponseTime')
                              "
                            />
                          </div>
                          ...
                          <div v-else-if="assertion.type === 'contains'">
                            ...
                            >
                              <el-input
                                v-model="assertion.expected"
                                :placeholder="
                                  $t('apiTesting.interface.expectedContains')
                                "
                                size="small"
                                style="flex: 1"
                              >
                                <template #append>
                                  <el-button
                                    ...
                                    :title="
                                      $t(
                                        'apiTesting.interface.referDataFactory',
                                      )
                                    "
                                    class="data-factory-btn"
                                  />
                              ...
                              <el-tooltip
                                :content="
                                  $t(
                                    'apiTesting.interface.insertDynamicVariable',
                                  )
                                "
                                placement="top"
                              >
                                <el-button
                                  ...
                                  @click="
                                    openVariableHelperForAssertion(
                                      assertion,
                                      'expected',
                                      index,
                                    )
                          ...
                          <div v-else-if="assertion.type === 'json_path'">
                            <el-input
                              ...
                              :placeholder="
                                $t('apiTesting.interface.jsonPathExample')
                              "
                              size="small"
                            ...
                            >
                              <el-input
                                v-model="assertion.expected"
                                :placeholder="
                                  $t('apiTesting.interface.expectedValue')
                                "
                                size="small"
                                style="flex: 1"
                              >
                          ...
                          <div v-else-if="assertion.type === 'header'">
                            <el-input
                              ...
                              :placeholder="
                                $t('apiTesting.interface.headerNameLabel')
                              "
                              size="small"
                              class="assertion-input"
                            />
                          ...
                          <div v-else-if="assertion.type === 'equals'">
                            ...
                            >
                              <el-input
                                v-model="assertion.expected"
                                :placeholder="
                                  $t('apiTesting.interface.expectedMatch')
                                "
                                size="small"
                                style="flex: 1"
                              >
                          ...
                          <!-- MongoDB 风格响应体匹配断言 -->
                          <div v-else-if="assertion.type === 'mongo_match'">
                            <div
                              style="
                                margin-bottom: 6px;
                                font-size: 12px;
                            ...
                            >
                              {{
                                $t(
                                  "apiTesting.interface.mongoMatchHint",
                                )
                              }}
                            </div>
                            <el-input
                              ...
                              type="textarea"
                              :rows="5"
                              :placeholder='{"code": "0", "data": {"token": {"$exists": true}, "count": {"$gt": 0}}}'
                              size="small"
                            />
                          </div>
                        </div>
                    ...
                    <div
                      v-if="
                        !selectedRequest.assertions ||
                        selectedRequest.assertions.length === 0
                      "
                      class="no-assertions"
                    >
            ...
            >
              <el-tab-pane label="Message" name="message">
                <div class="message-container">
                  <div class="message-input-section">
                    ...
                    >
                      <el-input
                        ...
                        :rows="6"
                        :placeholder="
                          $t('apiTesting.interface.inputWebSocketMessage')
                        "
                      />
                    </div>
                    ...
                    <div v-else-if="websocketMessageType === 'binary'">
                      ...
                      >
                        ...
                        <div class="el-upload__text">
                          {{ $t("apiTesting.interface.dragBinaryFile")
                          }}<em>{{
                            $t("apiTesting.interface.clickUpload")
                          }}</em>
                        </div>
                      </el-upload>
                      <div v-if="websocketBinaryFile" class="uploaded-file">
                        ...
                        <el-button
                          size="small"
                          type="danger"
                          @click="clearWebSocketBinaryFile"
                          >{{ $t("apiTesting.interface.clear") }}</el-button
                        >
                      </div>
                    </div>
                  ...
                  <!-- WebSocket消息历史记录 -->
                  <div
                    class="websocket-response-section"
                    v-if="websocketMessages.length > 0"
                  >
                      ...
                      >
                        <div class="message-header">
                          <span class="message-type" :class="msg.type">
                            {{
                              msg.type === "sent"
                                ? $t("apiTesting.interface.messageSent")
                                : msg.type === "connected"
                                  ? $t("apiTesting.interface.messageConnected")
                                  : msg.type === "info"
                                    ? $t("apiTesting.interface.messageInfo")
                                    : msg.type === "error"
                                      ? $t("apiTesting.interface.messageError")
                                      : $t(
                                          "apiTesting.interface.messageReceived",
                                        )
                            }}
                          </span>
                    ...
                    <div class="message-actions">
                      <el-button size="small" @click="clearWebSocketMessages">{{
                        $t("apiTesting.interface.clearHistory")
                      }}</el-button>
                    </div>
                  </div>
                </div>
          ...
          <div v-if="response" class="response-section">
                ...
                <span class="response-time"
                  >{{
                    response.response_time
                      ? response.response_time.toFixed(0)
                      : 0
                  }}ms</span
                >
              </div>
            ...
            <el-tabs v-model="responseActiveTab">
                ...
                <div class="response-body">
                  ...
                  <div v-if="showJsonPathExtractor" class="jsonpath-extractor">
                    <div class="jsonpath-input">
                      ...
                      >
                        <template #append>
                          <el-button size="small" @click="copyJsonPathResult">{{
                            $t("apiTesting.interface.copyResult")
                          }}</el-button>
                        </template>
                      </el-input>
                    </div>
                    <div v-if="jsonPathResult !== null" class="jsonpath-result">
                      <strong>{{
                        $t("apiTesting.interface.extractResult")
                      }}</strong>
                  ...
                  >
                    <div class="assertion-result-header">
                      ...
                      >
                        {{
                          result.passed
                            ? $t("apiTesting.interface.passed")
                            : $t("apiTesting.interface.failed")
                        }}
                      </el-tag>
                    ...
                    <div class="assertion-result-details">
                      <div class="result-row">
                        <span class="label">{{
                          $t("apiTesting.interface.expected")
                        }}</span>
                        <span class="value">{{
                      ...
                      <div class="result-row" v-if="result.error">
                        <span class="label">{{
                          $t("apiTesting.interface.error")
                        }}</span>
                        <span class="value error">{{ result.error }}</span>
                      </div>
                      <!-- MongoDB 匹配断言详情 -->
                      <div
                        v-if="result.mongo_result"
                        class="mongo-result-detail"
                      >
                        <div class="mongo-result-summary">
                          <el-tag type="info" size="small">
                            {{
                              $t("apiTesting.interface.mongoTotal")
                            }}: {{ result.mongo_result.total }}
                          </el-tag>
                          <el-tag type="success" size="small">
                            {{
                              $t("apiTesting.interface.mongoPassed")
                            }}: {{ result.mongo_result.successful_count }}
                          </el-tag>
                          <el-tag
                            :type="result.mongo_result.failed_count > 0 ? 'danger' : 'info'"
                            size="small"
                          >
                            {{
                              $t("apiTesting.interface.mongoFailed")
                            }}: {{ result.mongo_result.failed_count }}
                          </el-tag>
                        </div>
                        <div
                          v-if="result.mongo_result.failed_matches && result.mongo_result.failed_matches.length > 0"
                          class="mongo-failed-matches"
                        >
                          <div
                            v-for="(match, mi) in result.mongo_result.failed_matches"
                            :key="mi"
                            class="mongo-match-row failed"
                          >
              ...
              <el-tab-pane
                :label="$t('apiTesting.interface.console')"
                name="console"
    ...
    >
      ...
      >
        <el-form-item
          :label="$t('apiTesting.interface.collectionName')"
          prop="name"
        >
          <el-input
            v-model="collectionForm.name"
            :placeholder="$t('apiTesting.interface.inputCollectionName')"
          />
        </el-form-item>
        <el-form-item
          :label="$t('apiTesting.common.description')"
          prop="description"
        >
          <el-input
        ...
        <el-form-item
          :label="$t('apiTesting.interface.parentCollection')"
          prop="parent"
        >
          <el-select
            v-model="collectionForm.parent"
      ...
      <template #footer>
        <el-button @click="closeCreateCollectionDialog">{{
    ...
    >
      ...
      >
        ...
        >
          <el-select
            v-model="editCollectionForm.parent"
            :placeholder="$t('apiTesting.interface.selectParentCollection')"
            clearable
          >
            <el-option
              v-for="collection in flatCollections.filter(
                (c) => c.id !== editCollectionForm.id,
              )"
              :key="collection.id"
              :label="collection.name"
              :value="collection.id"
      ...
      <template #footer>
        <el-button @click="closeEditCollectionDialog">{{
          $t("apiTesting.common.cancel")
        }}</el-button>
    ...
    >
      <template v-if="!isBatchDeleteMode">
        <li @click="addRequest">
          {{ $t("apiTesting.interface.contextMenu.addRequest") }}
        </li>
        <li @click="addCollection">
          {{ $t("apiTesting.interface.contextMenu.addSubCollection") }}
        </li>
        <li
        ...
        <li @click="editNode">
          {{ $t("apiTesting.interface.contextMenu.edit") }}
        </li>
      </template>
      <li
        v-if="isBatchDeleteMode || (rightClickedNode && rightClickedNode.type === 'request')"
      ...
      <li @click="deleteNode">
        <template v-if="isBatchDeleteMode">
          {{ $t("apiTesting.interface.contextMenu.batchDelete", { count: selectedRequests.length }) }}
        </template>
        <template v-else>
          {{ $t("apiTesting.interface.contextMenu.delete") }}
        </template>
      </li>
    </ul>
    ...
    >
      <div
        v-if="variableCategories.length === 0"
        style="padding: 20px; text-align: center; color: #999"
      >
        <p>{{ $t("apiTesting.interface.variableCategoriesLoading") }}</p>
        <p>
          {{
            $t("apiTesting.interface.variableCategoriesCount", {
        ...
        >
          <div style="height: 450px; overflow-y: auto; padding: 10px">
            ...
            >
              <el-table-column
                prop="name"
                :label="$t('apiTesting.interface.functionName')"
                width="150"
              >
              ...
              <el-table-column
                prop="desc"
                :label="$t('apiTesting.interface.description')"
                min-width="150"
              />
              <el-table-column
                prop="syntax"
                :label="$t('apiTesting.interface.syntax')"
                min-width="200"
                show-overflow-tooltip
              />
              <el-table-column
                prop="example"
                :label="$t('apiTesting.interface.example')"
                min-width="200"
                show-overflow-tooltip
              />
              <el-table-column
                :label="$t('apiTesting.interface.operation')"
                width="80"
                fixed="right"
              >
                <template #default>
                  <el-button link type="primary" size="small">{{
                    $t("apiTesting.interface.insert")
                  }}</el-button>
                </template>
              </el-table-column>
            </el-table>
    ...
    <!-- Swagger 导入对话框 -->
    <el-dialog
      v-model="showSwaggerImportDialog"
      title="从 Swagger 导入接口"
      width="780px"
      :close-on-click-modal="false"
      @close="resetSwaggerImport"
    >
      <el-form :model="swaggerForm" label-width="100px">
        <el-form-item label="来源类型">
          <el-radio-group
            v-model="swaggerForm.sourceType"
            @change="swaggerPreviewList = []"
          >
            <el-radio value="url">Swagger URL</el-radio>
            <el-radio value="json">JSON 内容</el-radio>
          </el-radio-group>
        </el-form-item>

        <template v-if="swaggerForm.sourceType === 'url'">
          <el-form-item label="Swagger URL">
            <el-input
              v-model="swaggerForm.swaggerUrl"
              placeholder="https://example.com/api/v2/api-docs"
              clearable
            />
          </el-form-item>
          <el-form-item label="Token">
            <el-input
              v-model="swaggerForm.token"
              placeholder="Bearer token（可选，无需填写 Bearer 前缀）"
              clearable
            />
          </el-form-item>
        </template>
        ...
        <template v-else>
          <el-form-item label="JSON 内容">
            <div style="width: 100%">
              <el-upload
                :before-upload="handleSwaggerFileUpload"
                :show-file-list="false"
                accept=".json"
              ...
              <el-input
                v-model="swaggerForm.swaggerJson"
                type="textarea"
                :rows="8"
                placeholder="或直接粘贴 Swagger/OpenAPI JSON 内容..."
              />
            </div>
          </el-form-item>
        </template>
        ...
        <el-form-item label="目标项目">
          <el-select
            v-model="swaggerForm.projectId"
            placeholder="选择接口导入的目标项目"
            style="width: 100%"
          >
            <el-option
              v-for="p in projects"
          ...
          <div class="swagger-hint">
            接口将按 Tag
            自动归类到对应集合，每个接口自动生成正常、异常、边界值等多条用例
          </div>
        </el-form-item>
        ...
        <el-form-item label="更新模式">
          <el-switch
            v-model="swaggerUpdateMode"
            @change="swaggerPreviewList = []"
            active-text="更新已有用例"
            inactive-text="纯新增"
          />
          <div class="swagger-hint">
            开启后：已有用例的 URL/参数/断言将按 Swagger
            更新，新接口继续新增；用例名称和描述不会被覆盖
          </div>
        </el-form-item>

        <!-- 模块筛选（解析后显示） -->
        <el-form-item v-if="swaggerAllTags.length > 0" label="模块筛选">
          <div style="width: 100%">
            <div class="swagger-tag-toolbar">
              <el-checkbox
                :model-value="swaggerSelectedTags.length === swaggerAllTags.length"
                :indeterminate="
                  swaggerSelectedTags.length > 0 &&
                  swaggerSelectedTags.length < swaggerAllTags.length
                "
                @change="toggleAllTags"
              >
                全选（{{ swaggerSelectedTags.length }}/{{ swaggerAllTags.length }}）
              </el-checkbox>
            </div>
            <el-checkbox-group
              v-model="swaggerSelectedTags"
              class="swagger-tag-group"
            >
              <el-checkbox
                v-for="tag in swaggerAllTags"
                :key="tag"
                :value="tag"
              >
                {{ tag }}
      ...
      <!-- 预览区域 -->
      <div v-if="swaggerPreviewList.length > 0" class="swagger-preview">
        <div class="swagger-preview-header">
          <span
            >将生成
            <strong>{{ swaggerFilteredPreview.length }}</strong> 条用例</span
          >
          <span class="swagger-preview-stats">
            <el-tag size="small" type="success" style="margin-right: 4px"
              >{{ $t("apiTesting.swagger.normal") }}
              {{ swaggerPreviewStats.normal }}</el-tag
            >
            <el-tag size="small" type="danger" style="margin-right: 4px"
              >{{ $t("apiTesting.swagger.error") }}
              {{ swaggerPreviewStats.error }}</el-tag
            >
            <el-tag size="small" type="warning"
              >{{ $t("apiTesting.swagger.boundary") }}
              {{ swaggerPreviewStats.boundary }}</el-tag
            >
          </span>
        </div>
        <el-table
          :data="swaggerFilteredPreview.slice(0, 150)"
          max-height="300"
          size="small"
          border
        >
          <el-table-column
            v-if="swaggerUpdateMode"
            prop="action"
            label="操作"
            width="68"
          >
            <template #default="scope">
              <el-tag
                :type="scope.row.action === 'update' ? 'warning' : 'success'"
                size="small"
              >
                {{ scope.row.action === "update" ? "更新" : "新增" }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="case_type" label="用例类型" width="110">
            <template #default="scope">
              <el-tag
                :type="getCaseTypeTag(scope.row.case_type, scope.row.case_category)"
                size="small"
              >
                {{ scope.row.case_type }}
              </el-tag>
          ...
          <el-table-column prop="method" label="方法" width="75">
            <template #default="scope">
              <el-tag
                :type="getSwaggerMethodType(scope.row.method)"
                size="small"
              >
                {{ scope.row.method }}
              </el-tag>
        ...
        <div
          v-if="swaggerFilteredPreview.length > 150"
          class="swagger-preview-more"
        >
          仅展示前 150 条，实际将导入全部 {{ swaggerFilteredPreview.length }} 条用例
        </div>
      </div>

      <template #footer>
        <el-button @click="showSwaggerImportDialog = false">取消</el-button>
        <el-button @click="previewSwagger" :loading="swaggerParsing">
          解析预览
        </el-button>
        <el-button
          type="primary"
          @click="importSwagger"
          :loading="swaggerImporting"
          :disabled="swaggerFilteredPreview.length === 0"
        >
          {{ swaggerUpdateMode ? "生成/更新用例" : "生成用例" }}
          ({{ swaggerFilteredPreview.length }})
        </el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      />
      <template #footer>
        <el-button @click="closeCurlImportDialog">{{
          $t("apiTesting.common.cancel")
        }}</el-button>
        <el-button type="primary" @click="parseAndImportCurl">{{
          $t("apiTesting.interface.parseAndImport")
        }}</el-button>
      </template>
    </el-dialog>
    ...
    >
      ...
      />
      <template #footer>
        <el-button @click="closeCodeGenerateDialog">{{
          $t("apiTesting.common.cancel")
        }}</el-button>
        <el-button type="primary" @click="copyGeneratedCode">{{
...
// 按树的 DFS 顺序列出所有 request 节点，用于 Shift 范围选择
const flatRequestList = computed(() => {
  const result = []
  const walk = (nodes) => {
    for (const n of nodes) {
      if (n.type === 'request') result.push(n)
...
const selectedRequests = ref([])  // 已选中的 request 节点（Ctrl/Shift+Click 多选）
const anchorRequest = ref(null)   // Shift 范围选择的起始锚点

// 右键节点在多选中，且选中数 > 1，才进入批量删除模式
const isBatchDeleteMode = computed(() =>
  selectedRequests.value.length > 1 &&
  selectedRequests.value.some(r => r.id === rightClickedNode.value?.id)
)

// Swagger 导入相关
const showSwaggerImportDialog = ref(false);
const swaggerParsing = ref(false);
const swaggerImporting = ref(false);
const swaggerPreviewList = ref([]);
const swaggerAllTags = ref([]);
const swaggerSelectedTags = ref([]);
const swaggerUpdateMode = ref(false);
...
const loadCollections = async (projectId) => {
  try {
    ...
    // 加载请求（传入 projectId 过滤，避免跨项目数据混入）
    await loadRequests(projectId);
  } catch (error) {
...
const loadRequests = async (projectId) => {
  const pid = projectId || selectedProject.value;
  if (!pid) return;
...
const onNodeClick = async (data, event) => {
  if (data.type === 'request') {
    // Shift+Click：范围选中锚点到当前节点之间所有 request，不进入详情
    if (event && event.shiftKey) {
      event.preventDefault()
      const list = flatRequestList.value
      const anchor = anchorRequest.value
      if (anchor) {
        const fromIdx = list.findIndex(r => r.id === anchor.id)
        const toIdx = list.findIndex(r => r.id === data.id)
        if (fromIdx !== -1 && toIdx !== -1) {
          const [start, end] = fromIdx < toIdx ? [fromIdx, toIdx] : [toIdx, fromIdx]
          selectedRequests.value = list.slice(start, end + 1)
        }
      } else {
        selectedRequests.value = [data]
        anchorRequest.value = data
      }
      return
    }
    // Ctrl/Meta+Click：单点切换，不进入详情
  ...
  if (data.type === "request") {
    try {
      ...
      if (
        ...
        !Array.isArray(requestData.params)
      ) {
        const paramsArray = [];
      ...
      if (
        ...
        !Array.isArray(requestData.headers)
      ) {
        const headersArray = [];
...
const deleteNode = () => {
  ...
  // 批量删除：右键节点在多选中，且全部为 request 类型
  if (isBatchDeleteMode.value) {
    const count = selectedRequests.value.length;
    const confirmMsg = t('apiTesting.interface.contextMenu.confirmBatchDelete', { count });
    ElMessageBox.confirm(confirmMsg, t('apiTesting.interface.contextMenu.delete'), {
      ...
      .then(async () => {
        try {
          const ids = selectedRequests.value.map(r => r.id);
          await api.post('/api-testing/requests/batch-delete/', { ids });
          ElMessage.success(t('apiTesting.interface.contextMenu.batchDeleteSuccess', { count }));
          selectedRequests.value = [];
          await loadCollections(selectedProject.value);
          showContextMenu.value = false;
        } catch (error) {
...
const hasBody = computed(() => {
  return (
    ...
    ["POST", "PUT", "PATCH"].includes(selectedRequest.value.method)
  );
});
...
const highlightedResponseBody = computed(() => {
  ...
  try {
    ...
    return responseBody.value
      ...
      .replace(
        /:\s*(true|false|null)/g,
        ': <span style="color: #cb4b16;">$1</span>',
      )
      .replace(
        /:\s*([0-9]+(\.[0-9]+)?)/g,
        ': <span style="color: #d33682;">$1</span>',
      );
  } catch (e) {
    return responseBody.value;
  }
...
const sendRequest = async () => {
  ...
  try {
    ...
    const requestData = {
      ...
      skip_auth: selectedRequest.value.skip_auth || false,
    };
    ...
    const apiResponse = await api.post(
      `/api-testing/requests/${selectedRequest.value.id}/execute/`,
      requestData,
    );
  ...
  try {
    ...
    if (headersEditorRef.value) {
      ...
      if (
        ...
        Array.isArray(selectedRequest.value.headers)
      ) {
        finalHeaders = selectedRequest.value.headers.filter(
    ...
    const requestData = {
      ...selectedRequest.value,
      params: Array.isArray(selectedRequest.value.params)
        ? convertKeyValueArrayToObject(selectedRequest.value.params || [])
    ...
    if (selectedRequest.value.id) {
      response = await api.put(
        `/api-testing/requests/${selectedRequest.value.id}/`,
        requestData,
      );
    } else {
...
const cloneRequest = async () => {
  if (!selectedRequest.value || !selectedRequest.value.id) {
    ElMessage.warning("请先选择并保存一个接口用例");
    return;
  }
  await _doClone(selectedRequest.value.id);
};

// 右键菜单克隆
const cloneRequestFromContext = async () => {
  showContextMenu.value = false;
  const node = rightClickedNode.value;
  if (!node || node.type !== "request") return;
  await _doClone(node.id);
};

const _doClone = async (requestId) => {
  try {
    cloning.value = true;
    // 获取完整的接口数据
    const { data: src } = await api.get(`/api-testing/requests/${requestId}/`);
    ...
    if (created.collection) {
      if (!expandedKeys.value.includes(created.collection)) {
        expandedKeys.value.push(created.collection);
      }
    }
    selectedRequest.value = created;
    ElMessage.success(`接口「${src.name}」克隆成功`);
  } catch (error) {
...
const onAssertionTypeChange = (assertion) => {
  ...
  if (assertion.type === "status_code") {
    ...
    assertion.expected = "";
  } else if (assertion.type === "mongo_match") {
    assertion.expected = "";
  }
};
...
const formatResponse = () => {
  ...
  try {
    if (response.value.response_data.json) {
      response.value.response_data.json = JSON.parse(
        JSON.stringify(response.value.response_data.json),
      );
    }
...
const evaluateJsonPath = () => {
  if (
    ...
    !response.value.response_data.json ||
...
const updateCollection = async () => {
  ...
  try {
    await api.patch(
      `/api-testing/collections/${editCollectionForm.id}/`,
      {
...
// ===== Swagger 导入 =====
const openSwaggerImportDialog = () => {
  swaggerForm.projectId = selectedProject.value || null;
  swaggerPreviewList.value = [];
  swaggerAllTags.value = [];
  swaggerSelectedTags.value = [];
  swaggerUpdateMode.value = false;
  showSwaggerImportDialog.value = true;
};
...
const resetSwaggerImport = () => {
  ...
  swaggerSelectedTags.value = [];
  swaggerUpdateMode.value = false;
  swaggerForm.swaggerUrl = "";
  swaggerForm.token = "";
  swaggerForm.swaggerJson = "";
};

const toggleAllTags = (checked) => {
  swaggerSelectedTags.value = checked ? [...swaggerAllTags.value] : [];
};

const handleSwaggerFileUpload = (file) => {
  const reader = new FileReader();
  reader.onload = (e) => {
    swaggerForm.swaggerJson = e.target.result;
  };
  reader.readAsText(file);
  return false; // 阻止自动上传
};

const getSwaggerMethodType = (method) => {
  const map = {
    GET: "success",
    POST: "primary",
    PUT: "warning",
...
// 使用 case_category（稳定的英文 key）而非本地化的 case_type 文字做颜色判断
const getCaseTypeTag = (caseType, caseCategory) => {
  if (caseCategory === "normal") return "success";
  if (caseCategory === "boundary") return "warning";
  if (caseCategory === "error") return "danger";
  // 兜底：兼容旧数据的文字判断
  if (!caseType) return "info";
  if (caseType.startsWith("正常") || caseType.startsWith("Normal")) return "success";
  if (caseType.startsWith("边界") || caseType.startsWith("Boundary")) return "warning";
  return "danger";
};

// 按已选模块过滤后的预览列表（模块未选时显示全部）
const swaggerFilteredPreview = computed(() => {
  if (!swaggerSelectedTags.value.length) return swaggerPreviewList.value;
  return swaggerPreviewList.value.filter((c) =>
    swaggerSelectedTags.value.includes(c.tag),
  );
});

const swaggerPreviewStats = computed(() => {
  const stats = { normal: 0, boundary: 0, error: 0 };
  swaggerFilteredPreview.value.forEach((c) => {
    const cat = c.case_category || "error";
    stats[cat] = (stats[cat] || 0) + 1;
  });
  return stats;
});

const buildSwaggerPayload = (dryRun) => {
  const payload = {
    source_type: swaggerForm.sourceType,
    project_id: swaggerForm.projectId,
    dry_run: dryRun,
    // 空数组代表全部模块；dry_run 时不传 selected_tags 以获取完整 all_tags
    selected_tags: dryRun ? [] : swaggerSelectedTags.value,
    update_mode: swaggerUpdateMode.value,
    // 传递当前语言给后端，生成对应语言的用例名称和描述
    lang: locale.value.startsWith("en") ? "en" : "zh",
  };
  if (swaggerForm.sourceType === "url") {
    payload.swagger_url = swaggerForm.swaggerUrl.trim();
    payload.token = swaggerForm.token.trim();
  } else {
    payload.swagger_json = swaggerForm.swaggerJson.trim();
  }
  return payload;
};

const previewSwagger = async () => {
  if (!swaggerForm.projectId) {
    ElMessage.warning("请先选择目标项目");
    return;
  }
  if (swaggerForm.sourceType === "url" && !swaggerForm.swaggerUrl.trim()) {
    ElMessage.warning("请输入 Swagger URL");
    return;
  }
  if (swaggerForm.sourceType === "json" && !swaggerForm.swaggerJson.trim()) {
    ElMessage.warning("请粘贴或上传 Swagger JSON 内容");
    return;
  }
  ...
  try {
    const res = await api.post(
      "/api-testing/collections/import-swagger/",
      buildSwaggerPayload(true),
    );
    swaggerPreviewList.value = res.data.endpoints || [];
    swaggerAllTags.value = res.data.all_tags || [];
    // 默认全选所有模块
    swaggerSelectedTags.value = [...swaggerAllTags.value];
    if (swaggerPreviewList.value.length === 0) {
      ElMessage.warning("未解析到任何接口，请检查 Swagger 文档内容");
    } else {
      const newCnt = res.data.new_count ?? swaggerPreviewList.value.length;
      const updCnt = res.data.update_count ?? 0;
      const modeStr = swaggerUpdateMode.value
        ? `新增 ${newCnt} 条 / 更新 ${updCnt} 条`
        : `将生成 ${newCnt} 条`;
      ElMessage.success(
        `解析成功，共 ${swaggerAllTags.value.length} 个模块，${modeStr}用例`,
      );
    }
  } catch (err) {
    ElMessage.error(err.response?.data?.error || "解析失败，请检查输入内容");
  } finally {
...
const importSwagger = async () => {
  if (swaggerFilteredPreview.value.length === 0) return;
  swaggerImporting.value = true;
  try {
    const res = await api.post(
      "/api-testing/collections/import-swagger/",
      buildSwaggerPayload(false),
    );
    ElMessage.success(res.data.message || "导入成功");
    showSwaggerImportDialog.value = false;
    // 刷新集合树
    await loadCollections(swaggerForm.projectId);
  } catch (err) {
    ElMessage.error(err.response?.data?.error || "导入失败");
  } finally {
    swaggerImporting.value = false;
  }
};
// ===== End Swagger 导入 =====
...
const generateCode = async (language) => {
  ...
  try {
    ...
    const requestModel = {
      ...
      baseURL: baseURL,
      path: path,
      query: Array.isArray(selectedRequest.value.params)
        ? selectedRequest.value.params
        : [],
      headers: Array.isArray(selectedRequest.value.headers)
        ? selectedRequest.value.headers
        : [],
      body: {
        mode: bodyMode,
...
const toggleWebSocketConnection = async () => {
  ...
  if (websocketConnectionStatus.value === "connected") {
    ...
    try {
      ...
      const wsUrl = selectedRequest.value.url
        .replace("http://", "ws://")
        .replace("https://", "wss://");
...
const handleDataFactorySelect = (record) => {
  ...
  if (currentAssertion.value) {
    ...
    ElMessage.success(
      `${t("apiTesting.interface.referencedToAssertion")}: ${record.tool_name}`,
    );
  }
  // 如果是Body字段
...
const loadVariableFunctions = async () => {
  try {
    ...
    if (functionsData) {
      ...
      if (
        typeof groupedFunctions === "object" &&
        !Array.isArray(groupedFunctions)
      ) {
        variableCategories.value = Object.entries(groupedFunctions).map(
          ([label, variables]) => ({
            label,
            variables,
          }),
        ...
        const categoryMapping = {
          随机数: t(
            "apiTesting.component.keyValueEditor.categories.randomNumber",
          ),
          测试数据: t(
            "apiTesting.component.keyValueEditor.categories.testData",
          ),
          字符串: t("apiTesting.component.keyValueEditor.categories.string"),
          编码转换: t(
            "apiTesting.component.keyValueEditor.categories.encodingConversion",
          ),
          加密: t("apiTesting.component.keyValueEditor.categories.encryption"),
          时间日期: t(
            "apiTesting.component.keyValueEditor.categories.datetime",
          ),
          Crontab: t("apiTesting.component.keyValueEditor.categories.crontab"),
          未分类: t(
            "apiTesting.component.keyValueEditor.categories.uncategorized",
          ),
        };
      ...
      variables: [
        {
          ...
          desc: t("apiTesting.component.keyValueEditor.variables.randomInt"),
          example: "${random_int(100, 999, 1)}",
        },
        {
          name: "random_float",
          syntax: "${random_float(min, max, precision, count)}",
          desc: t("apiTesting.component.keyValueEditor.variables.randomFloat"),
        ...
        },
        {
          ...
          desc: t(
            "apiTesting.component.keyValueEditor.variables.randomBoolean",
          ),
        ...
        },
        {
          ...
          desc: t("apiTesting.component.keyValueEditor.variables.randomDate"),
      ...
      variables: [
        {
          ...
          desc: t("apiTesting.component.keyValueEditor.variables.randomString"),
          example: "${random_string(8, all, 1)}",
        },
        {
          name: "random_uuid",
          syntax: "${random_uuid(version, count)}",
          desc: t("apiTesting.component.keyValueEditor.variables.randomUuid"),
        ...
        },
        {
          ...
          desc: t(
            "apiTesting.component.keyValueEditor.variables.randomMacAddress",
          ),
        ...
        },
        {
          ...
          desc: t(
            "apiTesting.component.keyValueEditor.variables.randomIpAddress",
          ),
        ...
        },
        {
          ...
          desc: t(
            "apiTesting.component.keyValueEditor.variables.randomSequence",
          ),
...
.tree-node.is-multi-selected .node-label {
  color: #409eff;
}

.tree-node.is-multi-selected .el-icon {
  color: #409eff;
}
...
.method-select
  :deep(.el-select .el-input__wrapper .el-input__inner::placeholder) {
  background-color: transparent !important;
  color: white !important;
}
...
.method-select
  :deep(
    .el-select .el-input__wrapper .el-input__inner::-webkit-input-placeholder
  ) {
  background-color: transparent !important;
  color: white !important;
}
...
.method-select
  :deep(.el-select .el-input__wrapper .el-input__inner::-moz-placeholder) {
  background-color: transparent !important;
  color: white !important;
}

.method-select
  :deep(.el-select .el-input__wrapper .el-input__inner:-ms-input-placeholder) {
  background-color: transparent !important;
  color: white !important;
}
...
.skip-auth-toggle.active .skip-auth-label {
  color: #f56c6c;
}
