  ...
  <div class="key-value-editor">
    <div class="header">
      <div class="column key-column">
        {{ $t("apiTesting.component.keyValueEditor.key") }}
      </div>
      <div class="column value-column">
        {{ $t("apiTesting.component.keyValueEditor.value") }}
      </div>
      <div class="column description-column">
        {{ $t("apiTesting.component.keyValueEditor.description") }}
      </div>
      ...
      >
        ...
        <div class="column value-column">
          <el-input
            ...
            :show-password="isSensitiveKey(row.key)"
            size="small"
            @input="updateValue"
          >
            <template #append>
              <el-button
                ...
                :title="
                  $t('apiTesting.component.keyValueEditor.referDataFactory')
                "
          ...
          >
            <el-button size="small">{{
              $t("apiTesting.component.keyValueEditor.selectFile")
            }}</el-button>
          </el-upload>
          <el-tooltip
            :content="
              $t('apiTesting.component.keyValueEditor.insertDynamicVariable')
            "
            placement="top"
            v-if="!showFile || row.type !== 'file'"
          >
        ...
        >
          <div style="height: 450px; overflow-y: auto; padding: 10px">
            ...
            >
              <el-table-column
                prop="name"
                :label="$t('apiTesting.component.keyValueEditor.functionName')"
                width="150"
              ...
              <el-table-column
                prop="desc"
                :label="$t('apiTesting.component.keyValueEditor.desc')"
                min-width="150"
              />
              <el-table-column
                prop="syntax"
                :label="$t('apiTesting.component.keyValueEditor.syntax')"
                min-width="200"
                show-overflow-tooltip
              />
              <el-table-column
                prop="example"
                :label="$t('apiTesting.component.keyValueEditor.example')"
                min-width="200"
                show-overflow-tooltip
              />
              <el-table-column
                :label="$t('apiTesting.component.keyValueEditor.operation')"
                width="80"
                fixed="right"
              >
                <template #default>
                  <el-button link type="primary" size="small">{{
                    $t("apiTesting.component.keyValueEditor.insert")
                  }}</el-button>
                </template>
              </el-table-column>
            </el-table>
...
const updateValue = () => {
  ...
  const result = rows.value
    .filter((row) => row.key || row.value || row.description)
    .map((row) => ({
      key: row.key || "",
      value: row.value || "",
...
const handleDataFactorySelect = (record) => {
  ...
  if (record && record.output_data) {
    ...
    rows.value[rowIndex].description = t(
      "apiTesting.component.keyValueEditor.fromDataFactory",
...
const insertVariable = (variable) => {
  ...
  ElMessage.success(
    t("apiTesting.component.keyValueEditor.variableInserted", {
      name: variable.name,
    }),
  );
