  ...
  <div class="task-detail">
    <div class="page-header">
      <div class="header-left">
        ...
        <div class="task-info">
          <span class="task-id"
            >{{ $t("taskDetail.taskId") }}: {{ taskId }}</span
          >
          ...
          <template #title>
            <div class="collapse-title">
              <span class="title-text">{{
                $t("taskDetail.requirementTitle")
              }}</span>
              <span class="title-hint">{{
                $t("taskDetail.requirementHint")
              }}</span>
            </div>
          </template>
    ...
    <div v-else-if="!task.task_id" class="error-state">
      ...
      <router-link to="/ai-generation/generated-testcases">{{
        ...
        <div class="table-body">
          ...
          >
            ...
            <div class="body-cell">
              <span
                class="priority-tag"
                :class="testCase.priority?.toLowerCase()"
                >{{ testCase.priority || "P2" }}</span
              >
            </div>
            <div class="body-cell">
        ...
        <div class="pagination-controls">
          <div class="page-size-selector">
            ...
            <select v-model="pageSize" @change="currentPage = 1">
              <option value="10">
                {{ $t("taskDetail.pageSizeOption", { size: 10 }) }}
              </option>
              <option value="20">
                {{ $t("taskDetail.pageSizeOption", { size: 20 }) }}
              </option>
              <option value="50">
                {{ $t("taskDetail.pageSizeOption", { size: 50 }) }}
              </option>
            </select>
          </div>
      ...
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>
            {{
              isEditing
                ? $t("taskDetail.modalEditTitle")
                : $t("taskDetail.modalViewTitle")
            }}
          </h3>
        ...
        <div v-if="!isEditing" class="modal-body">
          <div class="detail-item">
            ...
            <span>{{
              selectedCase.caseId ||
              `TC${String(selectedCaseIndex + 1).padStart(3, "0")}`
            }}</span>
          </div>
          <div class="detail-item">
          ...
          <div class="detail-item">
            ...
            <p
              v-html="
                formatMarkdown(
                  selectedCase.precondition || $t('taskDetail.labelNone'),
                )
              "
            ></p>
          </div>
          <div class="detail-item">
            ...
            <p
              class="test-steps"
              v-html="formatMarkdown(selectedCase.steps)"
            ></p>
          </div>
          <div class="detail-item">
          ...
          <div class="detail-item">
            ...
            <span
              class="priority-tag"
              :class="selectedCase.priority?.toLowerCase()"
              >{{ selectedCase.priority || "P2" }}</span
            >
          </div>
        </div>
        ...
        <div v-else class="modal-body edit-mode">
          ...
          <div class="form-item">
            ...
            <el-input
              ...
              type="textarea"
              :rows="2"
              :placeholder="$t('taskDetail.placeholderScenario')"
            />
          </div>
          <div class="form-item">
            <label>{{ $t("taskDetail.labelPrecondition") }}</label>
            <el-input
              ...
              type="textarea"
              :rows="3"
              :placeholder="$t('taskDetail.placeholderPrecondition')"
            />
          </div>
          <div class="form-item">
            <label>{{ $t("taskDetail.labelSteps") }}</label>
            <el-input
              ...
              type="textarea"
              :rows="6"
              :placeholder="$t('taskDetail.placeholderSteps')"
            />
          </div>
          <div class="form-item">
            <label>{{ $t("taskDetail.labelExpected") }}</label>
            <el-input
              ...
              type="textarea"
              :rows="4"
              :placeholder="$t('taskDetail.placeholderExpected')"
            />
          </div>
          <div class="form-item">
            <label>{{ $t("taskDetail.labelPriority") }}</label>
            <el-select
              ...
              :placeholder="$t('taskDetail.placeholderPriority')"
            >
  ...
  computed: {
    isAllSelected() {
      return (
        ...
        this.selectedCases.length === this.testCases.length
      );
    },
  ...
  methods: {
    ...
    async loadTaskDetail() {
      try {
        ...
        const taskResponse = await api.get(
          `/requirement-analysis/testcase-generation/${this.taskId}/`,
        );
      ...
      if (isTableFormat && tableData.length > 1) {
        ...
        for (let i = 1; i < tableData.length; i++) {
          ...
          headers.forEach((header, index) => {
            ...
            if (
              cleanHeader === "优先级" ||
              cleanHeader === "priority" ||
              cleanHeader === "priority（优先级）" ||
              cleanHeader === "优先级（priority）"
            ) {
              ...
              if (
                !cleanHeader.includes("预期") &&
                !cleanHeader.includes("结果")
              ) {
    ...
    async batchAdopt() {
      if (this.selectedCases.length === 0) {
        ElMessage.warning(
          this.$t("taskDetail.pleaseSelectFirst", {
            action: this.$t("taskDetail.adopt"),
          }),
        );
        return;
      }
      ...
      try {
        await ElMessageBox.confirm(
          this.$t("taskDetail.confirmAdopt", {
            count: this.selectedCases.length,
          }),
      ...
      try {
        ...
        await api.post(
          `/requirement-analysis/testcase-generation/${this.taskId}/batch-adopt-selected/`,
          {
        ...
        ElMessage.success(
          this.$t("taskDetail.adoptSuccess", {
            count: this.selectedCases.length,
          }),
        );
        this.selectedCases = [];
        ...
        ElMessage.error(
          this.$t("taskDetail.batchAdoptFailed") +
            ": " +
    ...
    async batchDiscard() {
      if (this.selectedCases.length === 0) {
        ElMessage.warning(
          this.$t("taskDetail.pleaseSelectFirst", {
            action: this.$t("taskDetail.discard"),
          }),
        );
        return;
      }
      ...
      try {
        await ElMessageBox.confirm(
          this.$t("taskDetail.confirmDiscard", {
            count: this.selectedCases.length,
          }),
      ...
      try {
        // 获取选中用例的全局索引（不是分页索引）
        const caseIndices = this.selectedCases
          .map((selectedCase) => {
            // 在完整列表中查找索引
            const globalIndex = this.testCases.findIndex(
              (tc) =>
        ...
        const response = await api.post(
          `/requirement-analysis/testcase-generation/${this.taskId}/discard-selected-cases/`,
          {
        ...
        if (response.data.task_deleted) {
          ...
          ElMessage.success(
            this.$t("taskDetail.discardSuccess", {
              count: response.data.discarded_count,
            }),
          );
          ...
          if (response.data.updated_test_cases) {
            this.testCases = this.parseTestCases(
              response.data.updated_test_cases,
            );
            this.selectedCases = [];
        ...
        ElMessage.error(
          this.$t("taskDetail.batchDiscardFailed") +
            ": " +
    ...
    startEdit() {
      ...
      this.editForm = {
        ...
        // 将<br>转换为换行符以便编辑
        precondition: this.convertBrToNewline(
          this.selectedCase.precondition || "",
        ),
    ...
    async saveEdit() {
      ...
      try {
        ...
        await api.post(
          `/requirement-analysis/testcase-generation/${this.taskId}/update-test-cases/`,
          {
        ...
        ElMessage.error(
          this.$t("taskDetail.updateFailed") +
            ": " +
            (error.response?.data?.error || error.message),
        );
      } finally {
    ...
    async adoptSingleCase(testCase, index) {
      try {
        await ElMessageBox.confirm(
          this.$t("taskDetail.confirmAdoptSingle", {
    ...
    async discardSingleCase(testCase, index) {
      try {
        await ElMessageBox.confirm(
          this.$t("taskDetail.confirmDiscardSingle", {
            scenario: testCase.scenario,
          }),
          this.$t("taskDetail.confirmDiscardTitle"),
          {
      ...
      try {
        ...
        // 调用后端API弃用单个测试用例
        const response = await api.post(
          `/requirement-analysis/testcase-generation/${this.taskId}/discard-single-case/`,
          {
        ...
        ElMessage.error(
          this.$t("taskDetail.discardFailed") +
            ": " +
            (error.response?.data?.error || error.message),
        );
      }
    ...
    priorityToChinese(priority) {
      ...
      return (
        priorityMap[priority] || this.$t("generatedTestCases.priorityMedium")
      );
    },
    ...
    exportToExcel() {
      ...
      try {
        ...
        XLSX.utils.book_append_sheet(
          workbook,
          worksheet,
          this.$t("taskDetail.excelSheetName"),
        );

        // 生成文件名
        const dateStr = new Date().toISOString().slice(0, 10);
        const fileName = this.$t("taskDetail.excelFileName", {
          taskId: this.taskId,
          date: dateStr,
        });
        ...
        ElMessage.error(
          this.$t("taskDetail.exportFailed") + ": " + (error.message || ""),
        );
      } finally {
