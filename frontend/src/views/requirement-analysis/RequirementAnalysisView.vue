<template>
  <div class="requirement-analysis">
    ...
    <!-- 配置引导弹出窗口 -->
    <div
      v-if="showConfigGuide && !checkingConfig"
      class="modal-overlay"
      @click.self="showConfigGuide = false"
      :key="modalKey"
    >
        ...
        <div class="config-groups">
          ...
          <div class="config-group">
            ...
            <div class="config-items-row">
              <div
                class="config-item-inline"
                :class="getConfigItemClass('writer_model')"
              >
                <span
                  class="status-symbol"
                  v-html="getStatusSymbol('writer_model')"
                ></span>
                <span class="config-label">{{
                  $t("configGuide.caseWriter")
                }}</span>
                <span
                  class="config-name"
                  v-if="configStatus.writer_model.name"
                  >{{ configStatus.writer_model.name }}</span
                >
                <span
                  class="status-text"
                  v-if="!configStatus.writer_model.configured"
                  >{{ $t("configGuide.unconfigured") }}</span
                >
                <span
                  class="status-text warning"
                  v-else-if="!configStatus.writer_model.enabled"
                  >{{ $t("configGuide.disabled") }}</span
                >
              </div>

              <div
                class="config-item-inline"
                :class="getConfigItemClass('reviewer_model')"
              >
                <span
                  class="status-symbol"
                  v-html="getStatusSymbol('reviewer_model')"
                ></span>
                <span class="config-label">{{
                  $t("configGuide.caseReviewer")
                }}</span>
                <span
                  class="config-name"
                  v-if="configStatus.reviewer_model.name"
                  >{{ configStatus.reviewer_model.name }}</span
                >
                <span
                  class="status-text"
                  v-if="!configStatus.reviewer_model.configured"
                  >{{ $t("configGuide.unconfigured") }}</span
                >
                <span
                  class="status-text warning"
                  v-else-if="!configStatus.reviewer_model.enabled"
                  >{{ $t("configGuide.disabled") }}</span
                >
              </div>
            </div>
          ...
          <div class="config-group">
            ...
            <div class="config-items-row">
              <div
                class="config-item-inline"
                :class="getConfigItemClass('writer_prompt')"
              >
                <span
                  class="status-symbol"
                  v-html="getStatusSymbol('writer_prompt')"
                ></span>
                <span class="config-label">{{
                  $t("configGuide.caseWriter")
                }}</span>
                <span
                  class="config-name"
                  v-if="configStatus.writer_prompt.name"
                  >{{ configStatus.writer_prompt.name }}</span
                >
                <span
                  class="status-text"
                  v-if="!configStatus.writer_prompt.configured"
                  >{{ $t("configGuide.unconfigured") }}</span
                >
                <span
                  class="status-text warning"
                  v-else-if="!configStatus.writer_prompt.enabled"
                  >{{ $t("configGuide.disabled") }}</span
                >
              </div>
              ...
              <div
                class="config-item-inline"
                :class="getConfigItemClass('reviewer_prompt')"
              >
                <span
                  class="status-symbol"
                  v-html="getStatusSymbol('reviewer_prompt')"
                ></span>
                <span class="config-label">{{
                  $t("configGuide.caseReviewer")
                }}</span>
                <span
                  class="config-name"
                  v-if="configStatus.reviewer_prompt.name"
                  >{{ configStatus.reviewer_prompt.name }}</span
                >
                <span
                  class="status-text"
                  v-if="!configStatus.reviewer_prompt.configured"
                  >{{ $t("configGuide.unconfigured") }}</span
                >
                <span
                  class="status-text warning"
                  v-else-if="!configStatus.reviewer_prompt.enabled"
                  >{{ $t("configGuide.disabled") }}</span
                >
              </div>
            </div>
          ...
          <div class="config-group">
            <div class="group-label">
              {{ $t("configGuide.generationConfig") }}
            </div>
            <div class="config-items-row">
              <div
                class="config-item-inline"
                :class="getConfigItemClass('generation_config')"
              >
                <span
                  class="status-symbol"
                  v-html="getStatusSymbol('generation_config')"
                ></span>
                <span class="config-label">{{
                  $t("configGuide.generationSettings")
                }}</span>
                <span
                  class="config-name"
                  v-if="
                    configStatus.generation_config &&
                    configStatus.generation_config.name
                  "
                  >{{ configStatus.generation_config.name }}</span
                >
                <span
                  class="status-text"
                  v-if="
                    !configStatus.generation_config ||
                    !configStatus.generation_config.configured
                  "
                  >{{ $t("configGuide.unconfigured") }}</span
                >
              </div>
    ...
    <div class="output-mode-section" v-if="!isGenerating && !showResults">
      <div class="output-mode-card">
        ...
        <p class="mode-section-desc">
          {{ $t("requirementAnalysis.outputModeDesc") }}
        </p>
        <div class="output-mode-selector">
          <label
            class="mode-option"
            :class="{ active: globalOutputMode === 'stream' }"
          >
            ...
            <div class="mode-content">
              ...
              <div class="mode-desc">
                {{ $t("requirementAnalysis.realtimeStreamDesc") }}
              </div>
            </div>
          </label>
          <label
            class="mode-option"
            :class="{ active: globalOutputMode === 'complete' }"
          >
            <input type="radio" v-model="globalOutputMode" value="complete" />
            <div class="mode-content">
              <div class="mode-title">
                {{ $t("requirementAnalysis.completeOutput") }}
              </div>
              <div class="mode-desc">
                {{ $t("requirementAnalysis.completeOutputDesc") }}
              </div>
            </div>
          </label>
        </div>
    ...
    <div class="main-content">
        ...
        <div class="manual-input-card">
          ...
          <div class="input-form">
            <div class="form-group">
              <label
                >{{ $t("requirementAnalysis.requirementTitle") }}
            ...
            <div class="form-group">
              <label
                >{{ $t("requirementAnalysis.requirementDescription") }}
                <span class="required">*</span></label
              >
              <textarea
            ...
            <div class="form-group">
              ...
              <select v-model="manualInput.selectedProject" class="form-select">
                <option value="">
                  {{ $t("requirementAnalysis.selectProject") }}
                </option>
                <option
            ...
            >
              <span v-if="isGenerating">{{
                $t("requirementAnalysis.generating")
              }}</span>
        ...
        <div class="upload-card">
          ...
          >
            <div v-if="!selectedFile" class="upload-placeholder">
              ...
              <p class="upload-hint">
                {{ $t("requirementAnalysis.supportedFormats") }}
              </p>
              <input
                type="file"
                ref="fileInput"
        ...
        <div class="progress-card">
          ...
          <div class="progress-info">
            <div class="progress-item">
              ...
              <span class="value">{{
                currentTaskId || $t("requirementAnalysis.preparing")
              }}</span>
            </div>
            <div class="progress-item">
              <span class="label">{{
                $t("requirementAnalysis.currentStatus")
              }}</span>
              <span class="value">{{
                showResults
                  ? $t("requirementAnalysis.generationComplete")
                  : progressText
              }}</span>
            </div>
          </div>
          ...
          <div v-if="streamedContent" class="stream-content-display">
            <div class="stream-header">
              <span class="stream-title">{{
                $t("requirementAnalysis.realtimeGeneratedContent")
              }}</span>
              <span class="stream-status">{{
                $t("requirementAnalysis.characters", {
                  count: streamedContent.length,
                })
              }}</span>
            </div>
            <div
              class="stream-content"
              v-html="formatMarkdown(streamedContent)"
            ></div>
          </div>

          <!-- 评审内容显示区域 -->
          <div
            v-if="streamedReviewContent"
            class="stream-content-display"
            style="margin-top: 15px"
          >
            <div class="stream-header">
              <span class="stream-title">{{
                $t("requirementAnalysis.aiReviewComments")
              }}</span>
              <span class="stream-status">{{
                $t("requirementAnalysis.characters", {
                  count: streamedReviewContent.length,
                })
              }}</span>
            </div>
            <div
              class="stream-content"
              v-html="formatMarkdown(streamedReviewContent)"
            ></div>
          </div>
          ...
          >
            <div class="stream-header">
              <span class="stream-title">
                ...
                <span v-if="isGenerating" class="streaming-indicator">{{
                  $t("requirementAnalysis.generating")
                }}</span>
              </span>
              <span class="stream-status">{{
            ...
            <div
              class="stream-content final-testcases"
              v-html="formatMarkdown(finalTestCases)"
            ></div>
          </div>
          ...
          <div class="progress-steps">
            <div class="step" :class="{ active: currentStep >= 1 }">
              ...
              <span class="step-text">{{
                $t("requirementAnalysis.stepAnalysis")
              }}</span>
            </div>
            <div class="step" :class="{ active: currentStep >= 2 }">
              <span class="step-number">2</span>
              <span class="step-text">{{
                $t("requirementAnalysis.stepWriting")
              }}</span>
            </div>
            <div
              v-if="showReviewStep"
            ...
            >
              ...
              <span class="step-text">{{
                $t("requirementAnalysis.stepReview")
              }}</span>
            </div>
            <div
              class="step"
              :class="{ active: currentStep >= (showReviewStep ? 4 : 3) }"
            >
              <span class="step-number">{{ showReviewStep ? 4 : 3 }}</span>
              <span class="step-text">{{
                $t("requirementAnalysis.stepComplete")
              }}</span>
            </div>
          </div>
      ...
      <!-- 现在使用流式显示区域 + 最终版用例区域 -->
      <div
        v-if="false && showResults && generationResult"
        ...
        <div class="result-header">
          ...
          <div class="result-summary">
            <span class="summary-item">
              {{
                $t("requirementAnalysis.summaryTaskId", {
            ...
            <span class="summary-item">
              {{
                $t("requirementAnalysis.summaryGenerationTime", {
                  time: formatDateTime(generationResult.completed_at),
                })
              }}
            </span>
          </div>
...
export default {
  ...
  methods: {
    ...
    async checkConfigStatus() {
      try {
        ...
        if (
          ...
          if (
            response.data.generation_config &&
            response.data.generation_config.default_output_mode
          ) {
          ...
          if (
            response.data.generation_config &&
            response.data.generation_config.enable_auto_review !== null
          ) {
            this.showReviewStep =
              response.data.generation_config.enable_auto_review;
          } else {
    ...
    goToConfig() {
      ...
      if (
        !this.configStatus.generation_config ||
        !this.configStatus.generation_config.configured
      ) {
      ...
      if (
        !this.configStatus.writer_prompt.configured ||
        !this.configStatus.writer_prompt.enabled
      ) {
      ...
      if (
        !this.configStatus.writer_model.configured ||
        !this.configStatus.writer_model.enabled
      ) {
      ...
      if (
        !this.configStatus.reviewer_prompt.configured ||
        !this.configStatus.reviewer_prompt.enabled
      ) {
        this.$router.push("/configuration/prompt-config");
        return;
      }
      ...
      if (
        !this.configStatus.reviewer_model.configured ||
        !this.configStatus.reviewer_model.enabled
      ) {
        this.$router.push("/configuration/ai-model");
        return;
      }
    ...
    handleFileSelect(event) {
      ...
      if (file) {
        ...
        if (
          ...
          ElMessage.error(
            this.$t("requirementAnalysis.invalidFileFormatDetail"),
          );
        }
      }
    },
    ...
    async generateFromDocument() {
      ...
      try {
        ...
        const extractResponse = await api.get(
          `/requirement-analysis/documents/${uploadResponse.data.id}/extract_text/`,
        );
        ...
        console.error(
          this.$t("requirementAnalysis.documentProcessingFailed"),
          error,
        );
        ElMessage.error(
          this.$t("requirementAnalysis.documentProcessingFailed") +
    ...
    async startGeneration(
      ...
      try {
        ...
        const response = await api.post(
          "/requirement-analysis/testcase-generation/generate/",
          requestData,
        );
        ...
        ElMessage.error(
          this.$t("requirementAnalysis.createTaskFailed") +
            ": " +
            (error.response?.data?.error || error.message),
        );
    ...
    startStreamingProgress() {
      ...
      this.eventSource.onmessage = (event) => {
        ...
        try {
          ...
          if (data.type === "progress") {
            ...
            console.log(
              "📝 Received review content:",
              data.content.length,
              "characters",
              "Total length:",
              this.streamedReviewContent.length + data.content.length,
            );
            ...
            console.log(
              "🎯 Received final cases content:",
              data.content.length,
              "characters",
              "Total length:",
              this.finalTestCases.length + data.content.length,
            );
            this.finalTestCases += data.content;
            this.currentStep = 3;
            this.progressText =
              "🎯 " + this.$t("requirementAnalysis.statusRevising");
          } else if (data.type === "status") {
            // Final status
            console.log("📊 Received status update:", data.status);
            if (data.status === "completed") {
              this.progressText = this.$t(
                "requirementAnalysis.statusCompleted",
              );
              // Fetch final result
      ...
      this.eventSource.onerror = (error) => {
        ...
        if (this.eventSource.readyState === 2) {
          ...
          this.eventSource = null;
          ElMessage.warning(
            this.$t("requirementAnalysis.streamConnectionInterrupted"),
          );
          this.startPolling();
    ...
    async fetchFinalResult() {
      try {
        ...
        const response = await api.get(
          `/requirement-analysis/testcase-generation/${this.currentTaskId}/progress/`,
        );
        ...
        if (!this.hasShownCompletionMessage) {
          ElMessage.success(
            this.$t("requirementAnalysis.generateCompleteSuccess"),
          );
    ...
    startPolling() {
      this.pollInterval = setInterval(async () => {
        try {
          ...
          console.log(
            `${this.$t("requirementAnalysis.taskStatus")}: ${task.status}, ${this.$t("requirementAnalysis.progress")}: ${task.progress}%`,
          );
          ...
          if (task.status === "generating") {
            ...
            ElMessage.error(
              this.$t("requirementAnalysis.generateFailed") +
                ": " +
                (task.error_message ||
                  this.$t("requirementAnalysis.unknownError")),
            );
            return;
          }
        } catch (error) {
          console.error(
            this.$t("requirementAnalysis.checkProgressFailed"),
            error,
          );
          // 继续轮询，不中断
        }
      ...
      try {
        ...
        if (worksheetData.length > 1) {
          ...
          for (let row = 1; row < worksheetData.length; row++) {
            for (
              let col = 0;
              col < Math.min(6, worksheetData[row].length);
              col++
            ) {
        ...
        XLSX.utils.book_append_sheet(
          workbook,
          worksheet,
          this.$t("requirementAnalysis.testCaseSheetName"),
        );

        // 生成文件名（包含任务ID和日期）
        const fileName = this.$t("requirementAnalysis.excelFileName", {
          taskId: taskId,
        ...
        ElMessage.error(
          this.$t("requirementAnalysis.downloadFailed") +
            ": " +
            (error.message || this.$t("requirementAnalysis.unknownError")),
        );
      }
    },
    ...
    async saveToTestCaseRecords() {
      try {
        // 调用后端API保存到记录
        const response = await api.post(
          `/requirement-analysis/testcase-generation/${this.generationResult.task_id}/save_to_records/`,
        );
        ...
        if (response.data.already_saved) {
          ...
          ElMessage.success(
            `测试用例已保存！已导入 ${importedCount} 条测试用例到测试用例管理系统`,
          );
        }
        ...
        ElMessage.error(
          this.$t("requirementAnalysis.saveFailed") +
            ": " +
            (error.response?.data?.error || error.message),
        );
      }
      ...
      while (i < lines.length) {
        ...
        // 识别前置条件
          ...
          while (i < lines.length) {
            ...
            if (
              nextLine.includes("测试步骤") ||
              nextLine.includes("操作步骤") ||
              nextLine.includes("Test Steps") ||
              nextLine.includes("步骤") ||
              nextLine.includes("预期结果") ||
              nextLine.includes("Expected") ||
              nextLine.includes("优先级") ||
              nextLine.includes("Priority") ||
              nextLine.includes("测试用例") ||
              nextLine.includes("Test Case") ||
              nextLine.match(/^(\d+\.|\*|\-|\d+、)/)
            ) {
              break;
            }
        ...
        // 如果是没有明确标识的行，可能是场景描述的延续
          ...
          Object.keys(currentTestCase).length > 0 &&
          !currentTestCase.steps &&
          !currentTestCase.expected &&
          !currentTestCase.precondition
        ) {
