  ...
  <div class="page-container">
    ...
    <div class="card-container">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="4">
            <el-select
              ...
              :placeholder="$t('version.relatedProject')"
              clearable
              @change="handleFilter"
            >
              <el-option
          ...
          <el-col :span="3">
            <el-select
              ...
              :placeholder="$t('version.versionType')"
              clearable
              @change="handleFilter"
            >
      ...
      >
        ...
        >
          <template #default="{ row }">
            <div class="version-name">
              <span>{{ row.name }}</span>
              <el-tag
                v-if="row.is_baseline"
                type="warning"
                size="small"
                class="baseline-tag"
                >{{ $t("version.baseline") }}</el-tag
              >
            </div>
          </template>
        </el-table-column>
        <el-table-column
          prop="projects"
          :label="$t('version.relatedProject')"
          width="300"
        >
          <template #default="{ row }">
            <div
            ...
            >
              ...
              <el-tooltip
                v-if="row.projects.length > 2"
                :content="getProjectsTooltip(row.projects)"
              >
        ...
        />
        <el-table-column
          prop="testcases_count"
          :label="$t('version.testCaseCount')"
          width="100"
        >
          <template #default="{ row }">
        ...
        <el-table-column
          ...
          :label="$t('version.creator')"
          width="120"
        />
        <el-table-column
          prop="created_at"
          :label="$t('version.createdAt')"
          width="180"
        >
          <template #default="{ row }">
      ...
      >
        <el-form-item :label="$t('version.versionName')" prop="name">
          <el-input
            ...
            :placeholder="$t('version.versionNamePlaceholder')"
          />
        </el-form-item>
        ...
        <el-form-item>
          <el-checkbox v-model="versionForm.is_baseline">{{
            $t("version.setAsBaseline")
          }}</el-checkbox>
        </el-form-item>
      </el-form>
...
const versionRules = {
  name: [
    {
      required: true,
      message: computed(() => t("version.versionNameRequired")),
      trigger: "blur",
    },
  ],
  project_ids: [
    {
      required: true,
      message: computed(() => t("version.projectRequired")),
      trigger: "change",
    },
  ],
};
...
const batchDeleteVersions = async () => {
  ...
  try {
    ...
    if (successCount > 0) {
      ElMessage.success(
        t("version.batchDeleteSuccess", { successCount }) +
          (failCount > 0 ? `，${failCount} ${t("common.error")}` : ""),
      );
    } else {
