...
export default {
  ...
  dashboard: {
    ...
    features: {
      interfaceManagement: {
        title: "接口管理",
        description:
          "支持HTTP/HTTPS协议，快速导入Swagger/Postman数据，实现接口统一管理。",
      },
      automationTesting: {
        title: "自动化测试",
        description:
          "可视化的测试流程编排，支持断言、提取变量、前置/后置脚本等高级功能。",
      },
      scheduledTask: {
        title: "定时任务",
        description:
          "灵活的定时任务配置，支持Crontab表达式，实现无人值守的自动化回归测试。",
      },
      multiDimensionalReport: {
        title: "多维报告",
        description:
          "生成详细的测试报告，包含成功率、响应时间趋势等多维度数据分析。",
      },
    },
    timeFormat: {
      justNow: "刚刚",
  ...
  interface: {
    ...
    method: "方法",
    clone: "克隆",
    cloneInterface: "克隆该接口（生成副本）",
    skipAuth: "不携带Token",
    skipAuthOff: "当前：携带Token（点击可切换）",
    skipAuthOn: "当前：不携带Token（点击可切换）",
    importCurl: "导入CURL",
    exportCurl: "导出为CURL",
    ...
    pasteCurlCommand:
      "请粘贴CURL命令，例如：curl -X POST https://api.example.com/users -H 'Content-Type: application/json' -d 'name=test'",
    ...
    assertionTypes: {
      ...
      header: "响应头",
      equals: "完全匹配",
      mongoMatch: "响应体断言(MongoDB风格)",
    },
    mongoMatchHint: "支持 MongoDB 操作符：$eq $ne $gt $gte $lt $lte $in $nin $exists $size $regex\n示例：{\"code\": \"0\", \"data\": {\"token\": {\"$exists\": true}}}",
    mongoTotal: "断言总数",
    mongoPassed: "通过",
    mongoFailed: "失败",
    ...
    contextMenu: {
      ...
      edit: "编辑",
      delete: "删除",
      batchDelete: "删除（{count} 个）",
      moveToCollection: "移动到集合",
      confirmBatchDelete: "确定要删除所选的 {count} 个接口吗？",
      batchDeleteSuccess: "成功删除 {count} 个接口",
    },
    moveDialog: {
      title: "移动到集合",
  ...
  report: {
    ...
    selectExecutionTip:
      '请先在下方表格中选择一个测试执行记录，然后点击"生成并查看报告"按钮来生成并查看详细的测试报告',
    status: {
      pending: "待执行",
      running: "执行中",
      completed: "已完成",
  ...
  messages: {
    ...
    warning: {
      ...
      websocketNotSupported:
        "WebSocket接口暂不支持在此界面直接执行，请使用专门的WebSocket测试工具",
