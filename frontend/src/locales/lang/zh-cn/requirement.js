export default {
  requirementAnalysis: {
    ...
    outputModeDesc:
      "选择测试用例生成的输出方式（适用于手动输入和文档上传两种方式）",
  ...
  generationConfig: {
    ...
    timeoutHint:
      "AI评审和改进的总超时时间（建议：小文档120秒，大文档600-1800秒，超大文档可设置到3600秒）",
  ...
  knowledgeBase: {
    ...
    skillsPlaceholder: "## 通用规则\n每个功能点必须覆盖正常流程、异常流程、边界值三类场景。\n\n## 边界值规则\n- 数值字段必须覆盖最小值-1、最小值、最大值、最大值+1\n- 字符串字段必须覆盖空值、最大长度、超长输入\n\n## 业务约束\n- （示例）支付金额必须测试 0、负数、超大值（>999999）、小数精度\n- （示例）并发场景为高优先级，必须包含\n\n## 禁止项\n- （示例）不生成已知缺陷 #xxx 相关用例\n- （示例）跳过已下线的旧接口",
    saveBtn: "保存",
    previewBtn: "预览",
    saveSuccess: "保存成功",
    lastSaved: "最后保存：",
