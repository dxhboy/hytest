...
class VariableResolver:
    ...
    def set_variable(self, name: str, value: Any):
        ...
        print(f"[INFO] Variable set: {name} = {value}")

    def get_variable(self, name: str, default=None):
        """获取运行时变量"""
        return self.runtime_variables.get(name, default)
    ...
    def resolve(self, text):
        ...
        # 先解析运行时变量
        text = self._resolve_runtime_variables(text)
    ...
    def _resolve_runtime_variables(self, text):
        """解析运行时变量"""
        # 匹配 {{variable_name}} 模式
        pattern = r'\{\{([^}]+)\}\}'
    ...
    def _call_random_tool(self, func_name, args):
        ...
        if func_name == 'random_digits':
            result = RandomTools.random_string(length=args[0] if args else 6, char_type='digits',
                                               count=args[1] if len(args) > 1 else 1)
        elif func_name == 'random_letters':
            result = RandomTools.random_string(length=args[0] if args else 8, char_type='letters',
                                               count=args[1] if len(args) > 1 else 1)
        elif func_name == 'random_chinese':
            result = RandomTools.random_string(length=args[0] if args else 2, char_type='chinese',
                                               count=args[1] if len(args) > 1 else 1)
        elif func_name in tool_mapping:
            # 根据函数名设置默认参数
            kwargs = {}
            if func_name == 'random_int':
                kwargs = {'min_val': args[0] if len(args) > 0 else 0, 'max_val': args[1] if len(args) > 1 else 100,
                          'count': args[2] if len(args) > 2 else 1}
            elif func_name == 'random_float':
                kwargs = {'min_val': args[0] if len(args) > 0 else 0.0, 'max_val': args[1] if len(args) > 1 else 1.0,
                          'precision': args[2] if len(args) > 2 else 2, 'count': args[3] if len(args) > 3 else 1}
            elif func_name == 'random_string':
                kwargs = {'length': args[0] if len(args) > 0 else 8, 'char_type': args[1] if len(args) > 1 else 'all',
                          'count': args[2] if len(args) > 2 else 1}
                ...
                kwargs = {'start_date': args[0] if len(args) > 0 else '2024-01-01',
                          'end_date': args[1] if len(args) > 1 else '2024-12-31',
                          'count': args[2] if len(args) > 2 else 1,
                          'date_format': args[3] if len(args) > 3 else '%Y-%m-%d'}
                ...
                kwargs = {'length': args[0] if len(args) > 0 else 12, 'include_uppercase': True,
                          'include_lowercase': True, 'include_digits': True, 'include_special': True,
                          'count': args[1] if len(args) > 1 else 1}
                ...
                kwargs = {'sequence': sequence_arg, 'count': args[1] if len(args) > 1 else 1,
                          'unique': args[2] if len(args) > 2 else False}
    ...
    def _call_string_tool(self, func_name, args):
        ...
        if func_name == 'remove_whitespace':
            ...
            kwargs = {'text': args[0] if len(args) > 0 else '', 'old_str': args[1] if len(args) > 1 else '',
                      'new_str': args[2] if len(args) > 2 else '', 'is_regex': False}
        elif func_name == 'word_count':
            kwargs = {'text': args[0] if len(args) > 0 else ''}
        elif func_name == 'regex_test':
            kwargs = {'pattern': args[0] if len(args) > 0 else '', 'text': args[1] if len(args) > 1 else '',
    ...
    def _call_encoding_tool(self, func_name, args):
        ...
        if func_name == 'timestamp_convert':
            kwargs = {'timestamp': args[0] if len(args) > 0 else '',
                      'convert_type': args[1] if len(args) > 1 else 'to_datetime'}
            ...
            kwargs = {'text': args[0] if len(args) > 0 else '',
                      'convert_type': args[1] if len(args) > 1 else 'to_unicode'}
        elif func_name == 'ascii_convert':
            kwargs = {'text': args[0] if len(args) > 0 else '',
                      'convert_type': args[1] if len(args) > 1 else 'to_ascii'}
        elif func_name == 'color_convert':
            kwargs = {'color': args[0] if len(args) > 0 else '', 'from_type': args[1] if len(args) > 1 else 'hex',
                      'to_type': args[2] if len(args) > 2 else 'rgb'}
        elif func_name == 'base_convert':
            kwargs = {'number': args[0] if len(args) > 0 else 0, 'from_base': args[1] if len(args) > 1 else 10,
                      'to_base': args[2] if len(args) > 2 else 16}
            ...
            kwargs = {'image_data': args[0] if len(args) > 0 else '',
                      'image_format': args[1] if len(args) > 1 else 'png'}
        elif func_name == 'image_to_base64':
            kwargs = {'image_data': args[0] if len(args) > 0 else '',
                      'image_format': args[1] if len(args) > 1 else 'png', 'include_prefix': True}
    ...
    def _call_encryption_tool(self, func_name, args):
        ...
        if func_name in ['base64', 'md5', 'sha1', 'sha256', 'md5_hash', 'sha1_hash', 'sha256_hash', 'sha512_hash']:
            ...
            kwargs = {'text': args[0] if len(args) > 0 else '', 'hash_value': args[1] if len(args) > 1 else '',
                      'algorithm': args[2] if len(args) > 2 else 'md5'}
        elif func_name == 'aes_encrypt':
            kwargs = {'text': args[0] if len(args) > 0 else '', 'password': args[1] if len(args) > 1 else '',
                      'mode': args[2] if len(args) > 2 else 'CBC'}
        elif func_name == 'aes_decrypt':
            kwargs = {'encrypted_text': args[0] if len(args) > 0 else '', 'password': args[1] if len(args) > 1 else '',
                      'mode': args[2] if len(args) > 2 else 'CBC'}
    ...
    def parse_and_execute(self, script_text: str, context: dict = None):
        """解析字符串格式并执行对应的操作

        支持的格式：
        1. 变量赋值：variable=value
        2. 函数调用：${function_name()}
        3. 断言：assertions = "value" 或 assertions = {...} (支持多行 Python dict)

        Args:
            script_text: 多行字符串，每行一个操作
            context: 上下文变量字典（如response对象）
        ...
        # 第一步：提取多行 assertions = {...} / [...] 块
        remaining_text, extracted = self._extract_assertion_blocks(script_text)
        for val in extracted:
            results['assertions'].append({'expected': val, 'format': 'mongo'})
            print(f"[ASSERT] MongoDB style: {val}")

        # 第二步：逐行处理剩余脚本（单行断言 / 变量赋值 / 函数调用）
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            try:
                self._execute_line(line, context, results)
            except Exception as e:
                error_msg = f"第{line_num}行执行失败: {line} - {str(e)}"
                results['errors'].append(error_msg)
                print(f"[ERROR] {error_msg}")

        return results

    def _extract_assertion_blocks(self, script_text: str):
        """从脚本中提取多行 assertions = {...} / [...] 块，支持 Python dict 语法。

        使用 ast.literal_eval 解析，支持单引号、原始字符串 r"..."、尾随逗号等 Python 语法。
        返回 (剩余脚本文本, 提取的断言值列表)。
        """
        import ast as _ast
        ...
        while i < len(lines):
            ...
            m = re.match(r'^assertions\s*=\s*(.*)$', stripped)
            if m:
                value_part = m.group(1).strip()
                if value_part.startswith(('{', '[')):
                    # 逐行追加，直到 ast.literal_eval 解析成功
                    collected = value_part
                    parsed_val = None
                    j = i
                    while j < len(lines):
                        try:
                            parsed_val = _ast.literal_eval(collected)
                            break
                            ...
                            if j < len(lines):
                                ...
                                break  # 无法解析，放弃
                    if parsed_val is not None and isinstance(parsed_val, (dict, list)):
                        extracted.append(parsed_val)
                        i = j  # 跳过已消耗的行
                    else:
                        # 解析失败，保留原行留给正常处理
                        remaining.append(lines[i])
                else:
                    # 不是 dict/list，留给 _execute_line 的字符串断言处理
                    remaining.append(lines[i])
            else:
                remaining.append(lines[i])
            i += 1

        return '\n'.join(remaining), extracted

    def _execute_line(self, line: str, context: dict, results: dict):
        """执行单行命令"""

        # 处理断言格式：assertions = "value" 或 assertions = {...}
        if line.startswith('assertions'):
            # 解析断言
            match = re.match(r'assertions\s*=\s*(.+)$', line)
            if match:
                assertion_value = match.group(1).strip()
                # 先尝试解析为 JSON 对象（MongoDB 风格断言）
                import json as _json
                try:
                    json_val = _json.loads(assertion_value)
                    results['assertions'].append({
                        'expected': json_val,
                        'line': line,
                        'format': 'mongo'
                    })
                    print(f"[ASSERT] MongoDB style: {json_val}")
                except (ValueError, TypeError):
                    # 回退：去除引号，当字符串处理
                    if assertion_value.startswith(('"', "'")) and assertion_value.endswith(('"', "'")):
                        assertion_value = assertion_value[1:-1]
                    assertion_value = self._resolve_runtime_variables(assertion_value)
                    assertion_value = self.resolve(assertion_value)
                    results['assertions'].append({
                        'expected': assertion_value,
                        'line': line,
                        'format': 'string'
                    })
                    print(f"[ASSERT] Expected: {assertion_value}")

        # 处理变量赋值格式：variable=value
            ...
            # 解析值中的变量和函数
            var_value = self._resolve_with_context(var_value, context)

            # 设置为运行时变量
            self.set_variable(var_name, var_value)
            results['variables'][var_name] = var_value

        # 处理纯函数调用（可选）
        elif line.startswith('${') and line.endswith('}'):
            result = self._evaluate_expression(line[2:-1])
            print(f"[FUNC] {line} -> {result}")
            # 可以选择将结果保存到特殊变量
            self.set_variable('_last_result', result)

        else:
            # 尝试作为表达式解析
            try:
                result = self._evaluate_expression(line)
                print(f"[EXPR] {line} -> {result}")
                self.set_variable('_last_result', result)
            except:
                # 如果不是有效表达式，忽略
                pass

    def _resolve_with_context(self, text: str, context: dict) -> Any:
        """解析文本中的变量和函数，支持上下文对象

        支持格式：
        - response.json() - 调用上下文对象的方法
        - response.json().data.symbolName - 链式调用
        - ${random_name()} - 函数调用
        """
        if not isinstance(text, str):
            return text
        ...
        if '.' in text and not text.startswith('${'):
            ...
            if current is None:
                # 尝试作为运行时变量
                current = self.get_variable(parts[0])
                ...
                try:
                    for part in parts[1:]:
                        ...
                        if '(' in part and part.endswith(')'):
                            # 解析方法调用
                            method_name, args_str = part.split('(', 1)
                    ...
                    print(f"[WARNING] Failed to resolve chain: {text} - {e}")
                    # 继续尝试其他解析方式
    ...
    def compare_with_assertions(self, actual_value: Any, results: dict = None) -> bool:
        """比较实际值与断言结果

        Args:
            actual_value: 实际值
            results: 之前执行的结果字典（包含assertions）

        Returns:
            是否通过断言
        """
        if results is None or 'assertions' not in results:
            return True
        ...
        for assertion in results['assertions']:
            ...
            if actual_value == expected:
                print(f"[PASS] Assertion passed: {actual_value} == {expected}")
                assertion['result'] = 'passed'
            else:
                print(f"[FAIL] Assertion failed: {actual_value} != {expected}")
                assertion['result'] = 'failed'
                assertion['actual'] = actual_value
                all_passed = False

        return all_passed

    def execute_script_with_response(self, script_text: str, response) -> dict:
        """执行脚本并传入response对象
        ...
        results = self.parse_and_execute(script_text, context)

        # 如果有断言，可以后续进行比较
        results['assertion_passed'] = None
...
def parse_and_execute_script(script_text: str, context: dict = None):
    """便捷函数：解析并执行脚本"""
    return _resolver.parse_and_execute(script_text, context)


def execute_with_response(script_text: str, response):
    """便捷函数：使用response执行脚本"""
    return _resolver.execute_script_with_response(script_text, response)
